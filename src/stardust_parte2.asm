...
L_AC2C:
	...
	push de			;ac42
	push de			;ac43
	djnz L_AC2C		;ac44   ; Un solo `djnz` por cada 48 bytes: eso es lo que hace que salga cuatro veces mas barato que un `ldir`
...
rota_fondo_baja:		; El simetrico de rota_fondo_sube (lddr; 0xC462 = -2)
	...
	ld hl,08872h		;b168   ; Se empieza por el ultimo byte del tile (0x8872) y se guardan cuatro en la pila antes de correr el bloque
	ld e,(hl)			;b16b
	dec hl			;b16c
	ld d,(hl)			;b16d
	push de			;b16e
...
L_C63B:
	...
	pop de			;c63c
	pop hl			;c63d
	ld a,(ix+009h)		;c63e   ; El volumen que suena es el base mas lo que lleve la envolvente, recortado a cuatro bits
	add a,(ix+02ah)		;c641
	and 00fh		;c644
	ld (hl),a			;c646
	ld a,(ix+00ah)		;c647   ; Y el periodo va igual, pero a dos bytes: el `adc` arrastra el acarreo del bajo al alto
	add a,(ix+02bh)		;c64a
	ld (de),a			;c64d
	inc de			;c64e
	ld a,(ix+00bh)		;c64f
	...
	inc de			;c656
	push de			;c657
	ld de,0002eh		;c658   ; Al bloque del canal siguiente, 46 bytes mas alla
	add ix,de		;c65b
	pop de			;c65d
	pop af			;c65e
	inc a			;c65f   ; El numero de canal sube -es lo que mira el mezclador- y HL pasa al registro de volumen de al lado
	inc hl			;c660
	dec b			;c661
	jp nz,L_C517		;c662
	ld iy,0d0f2h		;c665   ; Y detras de los tres canales, la envolvente del RUIDO, que es una sola para todos y vive en las globales
	ld d,002h		;c669   ; Dos fases, y el mismo bucle que la de volumen
	ld c,000h		;c66b
L_C66D:
	ld a,(iy+000h)		;c66d   ; La espera de la fase...
	or a			;c670
	jr z,L_C67A		;c671
	dec a			;c673
	ld (iy+000h),a		;c674
	...
	jr L_C69B		;c678
L_C67A:
	ld a,(iy+002h)		;c67a   ; ...y los pasos que le quedan
	or a			;c67d
	jr z,L_C696		;c67e
	dec a			;c680
	ld (iy+002h),a		;c681
	ld a,(0d0feh)		;c684   ; El paso se suma en 0xD0FE, que es lo que la envolvente lleva puesto al periodo de ruido
	add a,(iy+006h)		;c687
	ld (0d0feh),a		;c68a
	ld a,(iy+008h)		;c68d
	ld (iy+000h),a		;c690
	...
	jr L_C69B		;c694
L_C696:
	inc iy		;c696   ; Fase agotada: a la otra
	dec d			;c698
	jr nz,L_C66D		;c699
L_C69B:
	ld a,c			;c69b
	or a			;c69c
	jr nz,L_C6A7		;c69d
	ld a,(0d0fch)		;c69f   ; Si ninguna hizo nada, el bit 2 de las banderas globales decide si se recarga y vuelve a empezar
	bit 2,a		;c6a2
	call nz,refresca_globales_sonido		;c6a4
L_C6A7:
	ld a,(0d0fdh)		;c6a7   ; Lo que llega al chip es el periodo de ruido que puso op_ruido mas lo de la envolvente
	ld e,a			;c6aa
	ld a,(0d0feh)		;c6ab
	add a,e			;c6ae
	ld (0d106h),a		;c6af
	call vuelca_psg		;c6b2   ; Y ahora si, los once registros de la sombra al PSG: una sola vez por interrupcion, con los tres canales ya resueltos
	pop af			;c6b5
	ret			;c6b6
carga_envolvente_1:		; Copia dos parejas de la plantilla del instrumento a las variables vivas; identica byte a byte a la de la fase de naves
	...
	ld d,002h		;c6b9
L_C6BB:
	ld a,(ix+020h)		;c6bb   ; Dos parejas: la espera desde el +20 y los pasos desde el +16
	ld (ix+00ch),a		;c6be
	ld a,(ix+016h)		;c6c1
	ld (ix+00eh),a		;c6c4
	inc ix		;c6c7   ; El `inc ix` pasa a la fase siguiente, que van byte a byte
	dec d			;c6c9
	jr nz,L_C6BB		;c6ca
	pop ix		;c6cc
	ret			;c6ce
carga_envolvente_2:		; Lo mismo con tres parejas; tambien identica
	...
	push ix		;c6d1
L_C6D3:
	ld a,(ix+022h)		;c6d3   ; Y aqui tres parejas, desde el +22 y el +18
	ld (ix+010h),a		;c6d6
	ld a,(ix+018h)		;c6d9
	ld (ix+013h),a		;c6dc
	inc ix		;c6df
...
refresca_globales_sonido:		; Refresca las variables globales del interprete desde 0xD0F2, que es 0xD068 + 3*46: justo detras del tercer estado de canal
	...
	ld iy,0d0f2h		;c6eb
L_C6EF:
	ld a,(iy+008h)		;c6ef   ; Las globales del ruido se rearman igual, con sus dos fases
	ld (iy+000h),a		;c6f2
	ld a,(iy+004h)		;c6f5
	ld (iy+002h),a		;c6f8
	inc iy		;c6fb
...
op_duracion:		; 0x83 n: (ix+006/007) = argumento por el tempo, con mul_a_de y el tempo leido de 0xD10B (`ld de,(0d10bh) / ld d,000h`: solo el byte bajo, porque el alto seria 0xD10C, que es el numero de canal). Es el valor de RECARGA, no la cuenta atras: quien lo copia a (ix+004/005) es 0xC570
	inc bc			;c70c
	ld a,(bc)			;c70d   ; La duracion que pide el guion...
	ld de,(0d10bh)		;c70e   ; ...por el tempo de 0xD10B, y eso son los cuadros que va a durar la nota
	ld d,000h		;c712
	call mul_a_de		;c714
	ld (ix+006h),l		;c717   ; Se guarda en el +06/+07, que es el valor de RECARGA
	ld (ix+007h),h		;c71a
	inc bc			;c71d
	jp L_C536		;c71e
op_tono_ruido:		; 0x81 n: (ix+008) = argumento and 0x09, que son los bits 0 y 3, la pareja tono/ruido del registro 7 del PSG. OJO AL SENTIDO: el mezclador de 0xC83E aplica el argumento COMPLEMENTADO, asi que un bit PUESTO enciende el generador. Lo confirma la musica, cuyos 28 argumentos son solo 0x01 (tono, 19 veces), 0x08 (ruido, 8) y 0x09 (los dos, 1)
	inc bc			;c721
	ld a,(bc)			;c722
	and 009h		;c723   ; Solo los bits 0 y 3: la pareja tono/ruido del canal en el registro 7
	ld (ix+008h),a		;c725
	inc bc			;c728
	jp L_C536		;c729
op_fin:		; 0x8B: cierra la voz. Borra los 46 bytes de su estado (`ld b,02eh`), pone a cero su transposicion en 0xD114+canal y repone el `di` (0xF3) en la cabecera de arranca_guion y arranca_guion_libre, deshaciendo el `ret` (0xC9) que planto sonido_off; y si el canal que acaba es el mismo que anoto op_efecto en 0xD0FF, limpia ademas 0xD0F2-0xD0FC y 0xD0FE, saltandose 0xD0FD -el periodo de ruido- porque el `ldir` es de diez. Es UN BYTE mas largo que el 0xE418 de naves, y esa es la razon de que los manejadores de aqui en adelante lleven delta 0x1CEB y los anteriores 0x1CEC
	push ix		;c72c
	pop hl			;c72e
	xor a			;c72f   ; El bloque del canal entero a cero, 46 bytes
	ld b,02eh		;c730
L_C732:
	ld (hl),a			;c732
	inc hl			;c733
	...
	xor (hl)			;c745
	jp nz,L_C63B		;c746
	ld hl,0d0f2h		;c749   ; ...se limpian ademas las globales del ruido: diez bytes con el `ldir` y uno mas saltandose 0xD0FD, que es el periodo y no se toca
	ld de,0d0f3h		;c74c
	ld bc,0000ah		;c74f
	ld (hl),a			;c752
	ldir		;c753
...
op_tempo:		; 0x85 n: 0xD10B = 6000 / (argumento * 8), con mul_a_de para el producto y la division de 0xC8BB, guardando solo el byte bajo del cociente. Es el multiplicador que aplica op_duracion, asi que a MAS argumento, tempo mas corto y musica mas rapida; sus diez apariciones son 0x32, 0xA0 (ocho veces) y 0xFF, que dan 15, 4 y 2
	inc bc			;c75a
	ld a,(bc)			;c75b   ; El argumento del tempo por ocho...
	push bc			;c75c
	ld de,00008h		;c75d
	call mul_a_de		;c760
	ld bc,01770h		;c763   ; ...y 6000 entre eso. A mas argumento, tempo mas corto y musica mas rapida
	push hl			;c766
	pop de			;c767
	call div_bc_de		;c768
	ld a,c			;c76b   ; Del cociente solo interesa el byte bajo
	ld (0d10bh),a		;c76c
	pop bc			;c76f
	inc bc			;c770
	jp L_C536		;c771
op_ruido:		; 0x88 n: 0xD0FD = argumento and 0x1F, los cinco bits del periodo de ruido del PSG, y de paso rearma la envolvente global con refresca_globales_sonido; lo que llega al chip es 0xD0FD + 0xD0FE, sumados en 0xC6A7. El BIT 7 del argumento decide por donde sigue el interprete: puesto, vuelve al bucle de comandos y solo cambia el parametro; a cero, cae en el ataque de 0xC558 y consume una duracion como si fuera una nota. De sus doce apariciones, once llevan el bit 7 a cero
	...
	ld a,(bc)			;c775
	push af			;c776
	and 01fh		;c777   ; Cinco bits: el periodo de ruido del PSG
	ld (0d0fdh),a		;c779
	call refresca_globales_sonido		;c77c   ; Y de paso se rearma la envolvente global
	pop af			;c77f
	inc bc			;c780
	or a			;c781   ; El bit 7 del argumento decide por donde sigue: puesto, vuelve al bucle de comandos; a cero, cae en el ataque y consume una duracion como si fuera nota
	jp m,L_C536		;c782
	jp L_C558		;c785
...
op_bucle:		; 0x82: recarga el puntero de ejecucion (ix+002/003) con el de inicio (ix+000/001) y vuelve a empezar el guion: dentro de una cancion es un bucle interno, NO un terminador. Solo cae en op_fin si se dan las dos condiciones de su cabecera, que el canal sea el 1 y que 0xD10D valga cero. Ese byte lo pone a 1 arranca_guion en 0xC4F2 cada vez que se lanza algo, y el unico que lo apaga es 0xB6AF, que justo despues se queda esperando a que se borren los dos primeros bytes del estado del canal 1: apagar 0xD10D es como se le pide a la musica que termine
	ld a,(0d10ch)		;c78c   ; Solo el canal 1 se pregunta si la musica tiene que acabarse...
	dec a			;c78f
	jr nz,L_C798		;c790
	ld a,(0d10dh)		;c792   ; ...y acaba de verdad cuando 0xD10D esta apagado, que es como se le pide el final
	and a			;c795
	jr z,op_fin		;c796
L_C798:
	ld c,(ix+000h)		;c798   ; Si no, el puntero de ejecucion vuelve al de inicio y el guion empieza otra vez
	ld b,(ix+001h)		;c79b
	ld (ix+002h),c		;c79e
	ld (ix+003h),b		;c7a1
	jp L_C536		;c7a4
op_banderas:		; 0x8A n: enciende bits con OR, sin apagar ninguno, y mete el argumento entero en las banderas del canal (ix+02D) y en las globales 0xD0FC. Son los bits de REPETICION de envolvente, y solo se leen tres en todo el bloque: el bit 0 hace que 0xC5C9 vuelva a llamar a carga_envolvente_1 cuando la envolvente de volumen se agota, el bit 1 que 0xC634 rellame a carga_envolvente_2 con la de tono, y el bit 2 que 0xC69F rellame a refresca_globales_sonido con la de ruido. Los apagan op_instrumento (`res 0` y `res 1`) y op_efecto (`res 2`)
	...
	ld a,(bc)			;c7a8
	ld e,a			;c7a9
	or (ix+02dh)		;c7aa   ; Las banderas se encienden con `or`: aqui no se apaga ninguna
	ld (ix+02dh),a		;c7ad
	ld a,(0d0fch)		;c7b0   ; Y las mismas, tambien en las globales
	or e			;c7b3
	ld (0d0fch),a		;c7b4
	inc bc			;c7b7
	jp L_C536		;c7b8
op_instrumento:		; 0x87 n: copia los quince bytes del instrumento n -que vive en 0xC8F7 + n*15- a (ix+016) y siguientes, que son las dos fases de la envolvente de volumen y las tres de la de tono, con su cuenta, su paso y su recarga cada una; de paso apaga los bits 0 y 1 de (ix+02D) y pone a cero los contadores (ix+00C/00D) y (ix+010/011/012) y los acumuladores (ix+02A) y (ix+02B/02C). La musica pide 47 veces los instrumentos 0 a 12 y el 15
	inc bc			;c7bb
	res 0,(ix+02dh)		;c7bc   ; Cambiar de instrumento apaga los dos bits de repeticion del canal
	res 1,(ix+02dh)		;c7c0
	ld a,(bc)			;c7c4
	ld de,0000fh		;c7c5   ; Quince bytes por instrumento, en 0xC8F7 + n*15
	call mul_a_de		;c7c8
	ld de,0c8f7h		;c7cb
	add hl,de			;c7ce
	push ix		;c7cf
	ld d,00fh		;c7d1
L_C7D3:
	ld a,(hl)			;c7d3   ; Se copian a partir del +16, que es donde vive la plantilla
	ld (ix+016h),a		;c7d4
	inc hl			;c7d7
	inc ix		;c7d8
	dec d			;c7da
	...
	pop ix		;c7de
	inc bc			;c7e0
	ld (ix+00ch),000h		;c7e1   ; Y los contadores vivos y los dos acumuladores, a cero: el instrumento nuevo empieza limpio
...
op_efecto:		; 0x89 n: arranca el barrido del RUIDO. Copia los seis bytes del efecto n -de 0xC9E7 + n*6- a las globales 0xD0F6-0xD0FB, que son las dos fases de la envolvente que va sumando en 0xD0FE; pone a cero 0xD0F2/0xD0F3 y el propio 0xD0FE, apaga el bit 2 de las banderas 0xD0FC -el que hace que la envolvente se recargue sola- y anota en 0xD0FF el canal que lo lanzo, que es a quien op_fin le pedira cuentas. La musica solo pide los efectos 0 y 3, y en el replay entero salta UNA vez
	inc bc			;c804
	ld a,(0d0fch)		;c805   ; Apaga el bit 2 para que la envolvente de ruido no se recargue sola
	res 2,a		;c808
	ld (0d0fch),a		;c80a
	ld a,(bc)			;c80d
	ld de,00006h		;c80e   ; Seis bytes por efecto, en 0xC9E7 + n*6
	call mul_a_de		;c811
	ld de,0c9e7h		;c814
	add hl,de			;c817
	ld iy,0d0f2h		;c818   ; Que van a parar al +04 de las globales, o sea 0xD0F6, dejando a cero las dos esperas de delante
...
L_C826:
	...
	jr nz,L_C826		;c82e
	xor a			;c830
	ld (0d0feh),a		;c831   ; Y lo que el ruido llevara sumado, a cero
	inc bc			;c834
	ld a,(0d10ch)		;c835   ; Se anota el canal que lo lanzo: es a quien op_fin le pedira cuentas
	ld (0d0ffh),a		;c838
	jp L_C536		;c83b
mezclador_canal:		; Enciende o apaga el tono y el ruido del canal que dice 0xD10C en la copia del registro 7 del PSG (0xD107): parte de D = 0x09 -bit 0 el tono y bit 3 el ruido del canal 0- y de E = complemento de A, los desplaza a la izquierda tantas veces como el numero de canal metiendo unos, y remata con `or d / and e`, o sea apaga la pareja entera y vuelve a encender solo lo que pide A. La llama tic_sonido dos veces por canal: con A=0 para callar el canal antes de leer el guion y con A=(ix+008) al atacar la nota. Gemela de la 0xE529 de la fase de naves
	push de			;c83e
	cpl			;c83f   ; El argumento entra COMPLEMENTADO: un bit puesto enciende el generador
	ld e,a			;c840
	ld d,009h		;c841   ; El 0x09 son los dos bits del canal 0, tono en el 0 y ruido en el 3
	ld a,(0d10ch)		;c843
L_C846:
	dec a			;c846   ; Y se corren tantas veces como diga el numero de canal...
	jp m,L_C851		;c847
	scf			;c84a
	rl e		;c84b   ; ...metiendo unos por debajo, para no tocar lo de los otros dos
	sla d		;c84d
	jr L_C846		;c84f
L_C851:
	ld a,(0d107h)		;c851   ; El `or d` apaga la pareja entera y el `and e` vuelve a encender solo lo que pide A
	or d			;c854
	and e			;c855
	ld (0d107h),a		;c856
	pop de			;c859
...
op_llama_frase:		; 0x8C n: el CALL del interprete. Guarda en 0xD10E + canal*2 el puntero al byte que sigue al operando y mete en BC la frase n leida de la tabla de veinte punteros de 0xCADC. Es de UN SOLO NIVEL -tres huecos de 2 B, uno por canal, y 0xD10E + 3*2 = 0xD114, donde ya empieza la tabla de transposiciones-, y a la musica le basta: es con diferencia el comando mas frecuente, 208 apariciones, y ninguna de las veinte frases lleva un 0x8C dentro
	ld a,(0d10ch)		;c864   ; El hueco de vuelta de este canal, dos bytes en 0xD10E + canal*2
	inc bc			;c867
	add a,a			;c868
	ld l,a			;c869
	ld h,000h		;c86a
	...
	ld de,0d10eh		;c86e
	add hl,de			;c871
	ld (hl),c			;c872   ; Ahi se guarda por donde iba el guion...
	inc hl			;c873
	ld (hl),b			;c874
	ld hl,0cadch		;c875   ; ...y BC se va a la frase, sacada de la tabla de veinte punteros de 0xCADC
...
op_vuelve:		; 0x8D: el RET del interprete, sin argumento. Recupera BC del hueco de 0xD10E + canal*2 que dejo op_llama_frase y sigue desde ahi. Por eso diecinueve de las veinte entradas de la tabla de frases acaban en 0x8D; la vigesima (0xCD53) acaba en 0x8B porque no es una frase sino un guion entero, el sonido que alta_objeto_acbb lanza al canal 2, y ninguno de los 208 comandos 0x8C la pide: todos van del 0 al 18
	ld a,(0d10ch)		;c880   ; El hueco de vuelta del canal...
	add a,a			;c883
	ld l,a			;c884
	ld h,000h		;c885
	ld de,0d10eh		;c887
	add hl,de			;c88a
	ld c,(hl)			;c88b   ; ...y BC vuelve por donde iba: un solo nivel, y a la musica le sobra
	inc hl			;c88c
	ld b,(hl)			;c88d
	jp L_C536		;c88e
...
mul_a_de:		; HL = A * DE por desplazamiento y suma; identica byte a byte a la del bloque de naves
	...
	ld b,008h		;c8ac
L_C8AE:
	srl a		;c8ae   ; Bit a bit del multiplicador: si el de abajo esta puesto, se suma...
	jr nc,L_C8B3		;c8b0
	add hl,de			;c8b2
L_C8B3:
	sla e		;c8b3   ; ...y el sumando se dobla en cada vuelta
	rl d		;c8b5
	djnz L_C8AE		;c8b7
	pop bc			;c8b9
	ret			;c8ba
div_bc_de:		; BC = BC / DE por restas y desplazamientos, dieciseis vueltas de `rl c / rla / adc hl,hl / sbc hl,de`, dejando el resto en HL y conservando AF; simulada contra 4000 divisiones al azar da el cociente y el resto exactos. Su unico cliente es op_tempo, que la llama con BC = 0x1770 = 6000 y DE = argumento*8. Gemela de la 0xE5A6 de la fase de naves
	...
	ld b,010h		;c8c0
L_C8C2:
	rl c		;c8c2   ; Dieciseis vueltas: se sube un bit al resto y se le resta el divisor...
	rla			;c8c4
	adc hl,hl		;c8c5
	sbc hl,de		;c8c7
	jr nc,L_C8CC		;c8c9   ; ...y si no cabia, se deshace la resta
	add hl,de			;c8cb
L_C8CC:
	ccf			;c8cc   ; El `ccf` es el que mete en el cociente el bit que dice si cabia
	djnz L_C8C2		;c8cd
	rl c		;c8cf
	rla			;c8d1
	ld b,a			;c8d2
...
lee_puntero:		; HL = la palabra que hay en HL + A*2; identica byte a byte a la del bloque de naves
	push af			;c8d5
	add a,a			;c8d6   ; El indice por dos, que las entradas de la tabla son palabras
	add a,l			;c8d7
	ld l,a			;c8d8
	jr nc,L_C8DC		;c8d9
	inc h			;c8db
L_C8DC:
	ld a,(hl)			;c8dc   ; Y la palabra, leida en dos pasos para no gastar mas registros
	inc hl			;c8dd
	ld h,(hl)			;c8de
	ld l,a			;c8df
	pop af			;c8e0
...
vuelca_psg:		; Vuelca al PSG los once bytes del bloque sombra 0xD100-0xD10A, que son los registros 0 a 10 del chip -los tres periodos de tono, el del ruido, el mezclador y los tres volumenes-, alternando el numero de registro por el puerto 0xA0 y el dato por el 0xA1. La llama tic_sonido una sola vez por interrupcion, al final, con los tres canales ya resueltos. Sus dieciocho ultimos bytes (0xC8E5-0xC8F6) son los de 0xE5D0 de la fase de naves sin tocar una coma; lo unico distinto son los tres del `ld hl,0d100h`
	ld hl,0d100h		;c8e2
	ld a,000h		;c8e5
	ld d,00bh		;c8e7   ; Once registros: los tres periodos de tono, el del ruido, el mezclador y los tres volumenes
L_C8E9:
	push af			;c8e9   ; Por el puerto 0xA0 va el numero de registro y por el 0xA1 el dato
	ld c,(hl)			;c8ea
	out (0a0h),a		;c8eb
	ld a,c			;c8ed
	out (0a1h),a		;c8ee
	pop af			;c8f0
	inc a			;c8f1   ; El registro siguiente y el byte siguiente de la sombra, que van en el mismo orden
	inc hl			;c8f2
	dec d			;c8f3
	jr nz,L_C8E9		;c8f4
	ret			;c8f6
...
vram_pon_dir:		; Fija la direccion de ESCRITURA del VDP por el puerto 0x99; identica byte a byte a la del bloque de naves
	di			;d117
	push af			;d118
	in a,(099h)		;d119   ; El `in a,(099h)` desengancha el latch del puerto antes de dar la direccion
	ld a,l			;d11b
	out (099h),a		;d11c
	ld a,h			;d11e
	and 07fh		;d11f   ; Bit 14 a uno y el 15 a cero: eso es lo que le dice al VDP que se va a ESCRIBIR
	or 040h		;d121
	out (099h),a		;d123
	pop af			;d125
	ret			;d126
premia:		; Suma B al marcador por el digito que apunta HL (dentro de 0xB87F) y lo repinta en 0x12B0, que es la misma posicion de pantalla que usa el marcador de la fase de naves
	push ix		;d127
	ld ix,0b87fh		;d129   ; El marcador vive en 0xB87F...
	ld de,012b0h		;d12d   ; ...y se repinta en 0x12B0, la misma posicion de pantalla que usa la fase de naves
	call suma_puntos		;d130
	pop ix		;d133
	ret			;d135
pinta_marca_hud:		; Pinta la marca del HUD con el color 0x11
	ld a,011h		;d136   ; (marca del HUD) El color 0x11 es negro sobre negro: la marca se pinta apagando la celda
	jr $+4		;d138

; ----------------------------------------------------------------------
; DATOS entrada_muerta_D13A: No es relleno: los dos bytes son 3E 71, o sea `ld
;   a,071h`, una SEGUNDA ENTRADA a la rutina de rellenar celdas de 0xD13C que
;   pintaria con el color 0x71 en vez de con el 0x11. No la llama nadie
;   -0xD13A no aparece ni una vez en el listado- y el `jr $+4` de 0xD138 esta
;   justo para saltarsela cuando se entra por pinta_marca_hud
;   0xd13a..0xd13c  (2 bytes)
DATA_entrada_muerta_D13A:
	defb 03eh,071h	; d13a
...
L_D13C:
	ld b,002h		;d13c   ; Dos celdas de ancho por dos de alto
L_D13E:
	push bc			;d13e
	push hl			;d13f
	ld c,002h		;d140   ; Y dos filas de caracter por vuelta
	call vram_pon_dir		;d142
L_D145:
	ld b,008h		;d145
L_D147:
	out (098h),a		;d147   ; Ocho bytes por celda, con los dos `and a` de respiro para el VDP
	and a			;d149
	and a			;d14a
	dec b			;d14b
	jr nz,L_D147		;d14c
	push af			;d14e
	ld a,l			;d14f   ; Al llegar a la ultima fila del tercio (0x38) hay que saltar al de abajo...
	and 038h		;d150
	cp 038h		;d152
	jr nz,L_D161		;d154
	ld a,l			;d156
	and 0c0h		;d157   ; ...poniendo la fila a cero y subiendo 0x800, que es el tamano del tercio
	ld l,a			;d159
	ld a,h			;d15a
	add a,008h		;d15b
	ld h,a			;d15d
...
L_D161:
	...
	jr nz,L_D145		;d163
	pop hl			;d165
	ld bc,00040h		;d166   ; Y la celda de al lado esta 0x40 mas alla: la columna siguiente
	add hl,bc			;d169
	pop bc			;d16a
	djnz L_D13E		;d16b
	ei			;d16d
...
redefine_teclas:		; La pantalla de REDEFINIR TECLAS: limpia el buffer, fuerza teclado, libera los nombres de tecla quitandoles el bit 7 y da ocho vueltas rotulando, con siete llamadas intercaladas que rellenan las siete entradas de 0xB86A. Gemela de la de naves, 34 de 48 bytes
	call borra_buffer		;d16f   ; La pantalla de redefinir teclas empieza limpia...
	call vuelca_pantalla		;d172
	xor a			;d175   ; ...y forzando teclado, que aqui el joystick no vale
	ld (0b87ch),a		;d176
	ld hl,0b7c2h		;d179   ; A los 72 nombres de tecla se les quita el bit 7: es la marca de "ya usada", y se sueltan todas
	ld b,048h		;d17c
L_D17E:
	res 7,(hl)		;d17e
	inc hl			;d180
	djnz L_D17E		;d181
	ld c,008h		;d183   ; Ocho vueltas: el titulo y las siete teclas
	ld ix,0b721h		;d185
	ld iy,0b86ah		;d189   ; IY recorre las siete entradas de 0xB86A, dos bytes cada una
	ld hl,00120h		;d18d
L_D190:
	push bc			;d190
	call rotula_secuencia		;d191   ; Cada vuelta rotula su trozo de texto...
	pop bc			;d194
	dec c			;d195   ; ...y las siete primeras piden ademas una tecla
	jr nz,redefine_tecla		;d196
	ld b,c			;d198
L_D199:
	dec bc			;d199   ; Y al acabar, una espera de 65536 antes de volver
	ld a,b			;d19a
	or c			;d19b
	jr nz,L_D199		;d19c
	ret			;d19e
lee_tecla_pulsada:		; Barre las nueve filas de la matriz y devuelve en E el indice fila*8 + bit de la primera tecla pulsada
	ld de,0f000h		;d19f   ; Las nueve filas de la matriz, desde la 0xF0
L_D1A2:
	ld a,d			;d1a2
	out (0aah),a		;d1a3   ; Fila por el puerto 0xAA y lectura por el 0xA9
	in a,(0a9h)		;d1a5
	ld b,008h		;d1a7
L_D1A9:
	rrca			;d1a9   ; Se rota hasta que salga un cero: ese es el bit de la tecla, y E lleva fila*8 + bit
	ret nc			;d1aa
	inc e			;d1ab
	djnz L_D1A9		;d1ac
	inc d			;d1ae   ; Y si en esta fila no habia nada, a la siguiente; agotadas las nueve, se vuelve a empezar
	ld a,0f9h		;d1af
	cp d			;d1b1
...
redefine_tecla:		; Espera una tecla y guarda su entrada en la tabla: la mascara del bit en (iy+000) y el valor del puerto en (iy+001)
	call lee_tecla_pulsada		;d1b6   ; Se espera a que pulsen
	xor a			;d1b9
	scf			;d1ba
L_D1BB:
	rra			;d1bb   ; De numero de bit a MASCARA: el `scf` mete un uno y las vueltas del `djnz` lo colocan
	djnz L_D1BB		;d1bc
	ld (iy+000h),a		;d1be   ; La mascara al +0 de la entrada y el numero de fila al +1
	inc iy		;d1c1
	ld (iy+000h),d		;d1c3
	inc iy		;d1c6
	ld d,b			;d1c8
	push hl			;d1c9
	ld hl,0b7c2h		;d1ca   ; El nombre de esa tecla, en la tabla de 0xB7C2
	add hl,de			;d1cd
	ld a,(hl)			;d1ce
	bit 7,a		;d1cf   ; Con el bit 7 puesto la tecla ya se uso: se deshacen los dos `inc iy` y a pedir otra
	jr z,L_D1DA		;d1d1
	dec iy		;d1d3
	dec iy		;d1d5
	pop hl			;d1d7
...
L_D1DA:
	set 7,(hl)		;d1da   ; Y si estaba libre, se marca como usada
	push ix		;d1dc   ; El hueco del rotulo, seis espacios
	pop hl			;d1de
	ld b,006h		;d1df
L_D1E1:
	ld (hl),020h		;d1e1
	inc hl			;d1e3
	djnz L_D1E1		;d1e4
	cp 015h		;d1e6   ; Los nombres de una sola letra se escriben tal cual...
	jr c,L_D1F0		;d1e8
	ld (ix+000h),a		;d1ea
	pop hl			;d1ed
	jr L_D190		;d1ee
L_D1F0:
	ld b,a			;d1f0   ; ...y los largos hay que buscarlos en la lista de 0xB80B, contando ceros
	ld hl,0b80bh		;d1f1
	jr L_D1FC		;d1f4
L_D1F6:
	ld a,(hl)			;d1f6
...
L_D1FC:
	...
	pop de			;d200
L_D201:
	ld a,(hl)			;d201   ; Y se copian hasta el cero
	cp 000h		;d202
	jr z,L_D20B		;d204
	ld (de),a			;d206
	inc hl			;d207
...
rotula_secuencia:		; Recorre la cadena de (IX) hasta el 0 pasandole cada byte a imprime_marco, con el sonido puesto a cero antes y despues
	call sonido_reset		;d20e   ; El sonido se calla antes de la cadena...
L_D211:
	ld a,(ix+000h)		;d211   ; ...que se va pasando caracter a caracter al impresor del marco
	inc ix		;d214
	and a			;d216
	jr z,sonido_reset		;d217   ; ...y se vuelve a callar al acabar
...
arranca_musica_carteles:		; Instala de golpe los tres guiones de la musica de los carteles, los dos primeros sin `ei`. La llama el rotulador como codigo de control
	ld a,080h		;d21e   ; Los tres guiones de la musica de los carteles, seguidos: los dos primeros con el bit 7 para no reactivar las interrupciones a medias
	ld de,0ce2bh		;d220
	push hl			;d223
	...
	ld a,002h		;d22e   ; Y el tercero sin el bit 7, que es el que devuelve las interrupciones
	ld de,0d05eh		;d230
...
sonido_reset:		; Reabre el sonido reponiendo el `di` (0xF3) que sonido_off machaco en las dos rutinas de arranque, y deja los tres canales a cero con un guion nulo
	ld a,0f3h		;d238   ; El `di` repuesto en las dos cabeceras: esto es lo que reabre la puerta que cerro sonido_off
	ld (0c4d0h),a		;d23a
	ld (0c4a3h),a		;d23d
	xor a			;d240   ; Y los tres canales, arrancados con un guion NULO: quedan a cero y callados
	ld de,00000h		;d241
	push hl			;d244
...
imprime_marco:		; El impresor del marco: DIRECTO a la VRAM con la fuente de 0x5F00; A<0x20 es control (0 borra la celda, 1 posiciona con el parametro de (IX))
	ld bc,00bb8h		;d252   ; 3000 vueltas antes de cada caracter: el texto sale a maquina de escribir
	push af			;d255
	call espera_bc		;d256
	pop af			;d259
	cp 020h		;d25a   ; De 0x20 para arriba es caracter; por debajo, control
	jr nc,L_D2D3		;d25c
	call vram_pon_dir		;d25e   ; Sea cual sea el control, lo primero es borrar la celda donde esta el cursor
	ld b,008h		;d261
	push af			;d263
L_D264:
	xor a			;d264
	...
	ei			;d26b
	pop af			;d26c
	cp 001h		;d26d   ; El control 1 coloca...
	jr nz,L_D28B		;d26f
	ld a,h			;d271   ; ...conservando el tercio y la fila de caracter y tirando la columna...
	and 018h		;d272
	ld h,a			;d274
	ld a,l			;d275
	and 038h		;d276
	ld l,a			;d278
	ld a,(ix+000h)		;d279   ; ...para sumarle la que trae el propio texto, por 0x40, que es lo que ocupa una columna
	inc ix		;d27c
	push hl			;d27e
	ld l,a			;d27f
	ld h,000h		;d280
...
L_D284:
	...
	jr L_D2F2		;d289
L_D28B:
	cp 014h		;d28b   ; El control 0x14 es la pausa larga: silencio, dos esperas de 65536 y la musica otra vez
	jr nz,L_D29E		;d28d
...
L_D29E:
	ld a,h			;d29e   ; Los demas controles son SALTO DE LINEA: se conservan tercio y fila y se tira la columna
	and 018h		;d29f
	ld h,a			;d2a1
	ld a,l			;d2a2
	and 038h		;d2a3
	ld l,a			;d2a5
	cp 038h		;d2a6   ; Si era la ultima fila del tercio...
	jr nz,L_D2B2		;d2a8
	ld a,h			;d2aa
	add a,009h		;d2ab   ; ...se salta al tercio siguiente, que es 0x800, mas 0x100 de las cuatro columnas del margen
	ld h,a			;d2ad
	ld l,000h		;d2ae
	jr L_D2B6		;d2b0
L_D2B2:
	ld bc,00108h		;d2b2   ; Y si no, una fila de caracteres mas abajo y a la columna 4
	add hl,bc			;d2b5
L_D2B6:
	ld b,007h		;d2b6
	call vram_pon_dir		;d2b8
L_D2BB:
	ld a,07fh		;d2bb   ; El cursor: siete filas de 0x7F
	out (098h),a		;d2bd
	and a			;d2bf
	and a			;d2c0
	djnz L_D2BB		;d2c1
	ei			;d2c3
	push hl			;d2c4
	call sonido_reset		;d2c5   ; Cada salto de linea calla el sonido, espera y vuelve a arrancar la musica de los carteles
	ld bc,00fa0h		;d2c8
	call espera_bc		;d2cb
...
L_D2D3:
	push hl			;d2d3
	ld l,a			;d2d4   ; El glifo del caracter, en 0x5F00 + caracter*8
	ld h,000h		;d2d5
	add hl,hl			;d2d7
	add hl,hl			;d2d8
	add hl,hl			;d2d9
	ld de,05f00h		;d2da   ; La fuente vive en 0x5F00, la misma que usa todo el bloque
	add hl,de			;d2dd
	ex de,hl			;d2de
	pop hl			;d2df
	...
	ld b,008h		;d2e3
L_D2E5:
	ld a,(de)			;d2e5   ; Ocho bytes directos a la VRAM
	out (098h),a		;d2e6
	inc de			;d2e8
	and a			;d2e9
	and a			;d2ea
	djnz L_D2E5		;d2eb
	ei			;d2ed
	ld bc,00040h		;d2ee   ; Y el cursor se pinta en la celda de al lado, 0x40 mas alla
	add hl,bc			;d2f1
L_D2F2:
	call vram_pon_dir		;d2f2
	ld b,007h		;d2f5
L_D2F7:
	ld a,07fh		;d2f7   ; Siete filas de 0x7F otra vez...
	out (098h),a		;d2f9
	and a			;d2fb
	and a			;d2fc
	djnz L_D2F7		;d2fd
	ei			;d2ff
	ld b,000h		;d300   ; ...y un retardo de 256 vueltas antes de soltar el caracter siguiente
L_D302:
	djnz L_D302		;d302
	ret			;d304
...
hay_tecla:		; Vuelve con NZ si hay alguna tecla pulsada, barriendo las nueve filas de la matriz por los puertos 0xAA y 0xA9
	ld d,0f0h		;d30b   ; Las nueve filas de la matriz, desde la 0xF0
L_D30D:
	ld a,d			;d30d
	out (0aah),a		;d30e
	in a,(0a9h)		;d310
	cp 0ffh		;d312   ; Con 0xFF no hay ninguna tecla pulsada en esa fila
	ret nz			;d314
	inc d			;d315
	ld a,0f9h		;d316   ; Y agotadas las nueve se vuelve con Z: no hay nada
	cp d			;d318
	ret z			;d319
	jr L_D30D		;d31a
rellena_colores:		; Pone el color A en la tabla de colores, tercio a tercio: 0x2108 x 56 filas, 0x2900 x 64 y 0x3100 x 40
	ld hl,02108h		;d31c   ; La tabla de colores va por tercios, igual que la de patrones
	ld c,038h		;d31f   ; 56 filas de pixel el primer tercio, 64 el segundo y 40 el tercero: las mismas tres bandas del volcado
...
rellena_tercio:		; El nucleo de rellena_colores: 24 filas de C bytes cada una, avanzando 0x40 en la VRAM entre fila y fila
	ld b,018h		;d331   ; Y 24 columnas por tercio, que es el ancho del area
L_D333:
	call vram_pon_dir		;d333
	push bc			;d336
	push hl			;d337
L_D338:
	out (098h),a		;d338   ; Los bytes de la columna, con el `and a` de respiro para el VDP
	and a			;d33a
	dec c			;d33b
	jr nz,L_D338		;d33c
	pop hl			;d33e
	ld bc,00040h		;d33f   ; Y de columna a columna, 0x40
	add hl,bc			;d342
	pop bc			;d343
	djnz L_D333		;d344
	ei			;d346
...
hud_imprime:		; Recorre la cadena de indices de glifo de (IX) hasta el 0 estampando cada uno en la VRAM: 8 bytes desde la fuente de 0x5F00 y 0x40 de avance por caracter
	ld a,(ix+000h)		;d35b   ; La cadena de indices de glifo, hasta el cero
	and a			;d35e
	ret z			;d35f
	ld l,a			;d360   ; El glifo en 0x5F00 + indice*8
	ld h,000h		;d361
	add hl,hl			;d363
	add hl,hl			;d364
	add hl,hl			;d365
	...
	ld b,008h		;d36e
L_D370:
	ld a,(de)			;d370   ; Ocho bytes a la VRAM, con dos `and a` de respiro entre uno y otro
	out (098h),a		;d371
	inc de			;d373
	and a			;d374
	and a			;d375
	djnz L_D370		;d376
	ei			;d378
	ld de,00040h		;d379   ; Y el caracter siguiente, 0x40 mas alla: la columna de al lado
	add hl,de			;d37c
	ex de,hl			;d37d
	inc ix		;d37e
	jp hud_imprime		;d380
vuelca_pantalla:		; Vuelca el buffer a la VRAM en TRES bandas: 0x4000/56 filas a 0x0108, 0x4540/64 a 0x0900, 0x4B40/40 a 0x1100
	ld de,04000h		;d383   ; La banda de arriba: 56 filas del buffer a 0x0108
	ld hl,00108h		;d386
	ld b,038h		;d389
	call vuelca_columnas		;d38b
	ld de,04540h		;d38e   ; La de en medio, 64 filas a 0x0900...
	ld hl,00900h		;d391
	ld b,040h		;d394
	call vuelca_columnas		;d396
	ld de,04b40h		;d399   ; ...y la de abajo, 40 a 0x1100. Las tres empiezan en la columna 4
	ld hl,01100h		;d39c
	ld b,028h		;d39f
...
vuelca_columnas:		; El nucleo del volcado, identico al de la fase de naves: 24 vueltas por banda recogiendo el buffer a saltos de 24 -por columnas- y avanzando 0x40 en la VRAM entre una y otra
	ld c,018h		;d3a6   ; 24 columnas por banda
L_D3A8:
	push bc			;d3a8
	call vram_pon_dir		;d3a9   ; Una columna por vuelta, dando la direccion de VRAM cada vez
	push de			;d3ac
	push hl			;d3ad
	ex de,hl			;d3ae
	ld de,00018h		;d3af
L_D3B2:
	ld a,(hl)			;d3b2   ; El buffer se recoge a saltos de 24 -o sea por columnas- porque asi es como la VRAM las quiere seguidas
	out (098h),a		;d3b3
	add hl,de			;d3b5
	djnz L_D3B2		;d3b6
	pop hl			;d3b8
	ld de,00040h		;d3b9   ; Y de columna a columna, 0x40 en la VRAM y uno en el buffer
	add hl,de			;d3bc
	pop de			;d3bd
	inc de			;d3be
	pop bc			;d3bf
...
L_D3C4:
	...
	ld ix,0b940h		;d3ca   ; El cartel de BRAVO y lo que sigue
	ld hl,00118h		;d3ce
	call rotula_secuencia		;d3d1
	pop de			;d3d4   ; La ficha del record la dejo en la pila entra_en_records
	push hl			;d3d5
	ld h,d			;d3d6   ; Y se rellena de espacios: seis caracteres
	ld l,e			;d3d7
	inc de			;d3d8
	ld bc,00005h		;d3d9
	ld (hl),020h		;d3dc
	...
	sbc hl,bc		;d3e4
	ex de,hl			;d3e6
	ld b,006h		;d3e7   ; Seis caracteres es lo que cabe en el nombre
	pop hl			;d3e9
L_D3EA:
	push bc			;d3ea
	push de			;d3eb
L_D3EC:
	call hay_tecla		;d3ec   ; Primero se espera a que suelten la tecla anterior...
	jr nz,L_D3EC		;d3ef
	ld bc,007d0h		;d3f1   ; ...y luego a que pulsen otra
	call espera		;d3f4
	call lee_tecla_pulsada		;d3f7
	ld d,000h		;d3fa
	push hl			;d3fc
	ld hl,0b7c2h		;d3fd   ; El nombre de la tecla, en la tabla de 0xB7C2
	add hl,de			;d400
	ld a,(hl)			;d401
	pop hl			;d402
	and 07fh		;d403
	cp 030h		;d405   ; De 0x30 para arriba entra tal cual
	jr nc,L_D41B		;d407
	cp 014h		;d409   ; Y de las de abajo solo valen tres: 0x14 borra...
	jr z,L_D43C		;d40b
	cp 011h		;d40d   ; ...0x11 mete un espacio...
	jr z,L_D419		;d40f
	cp 010h		;d411   ; ...y 0x10 cierra el nombre. Lo demas se ignora
	jr nz,L_D3EC		;d413
	pop de			;d415
	pop bc			;d416
	jr L_D428		;d417
...
L_D41B:
	...
	pop bc			;d41c
	push bc			;d41d
	ld (de),a			;d41e   ; El caracter, a la ficha, y al marco
	push de			;d41f
	...
	djnz L_D3EA		;d426
L_D428:
	ld a,00dh		;d428   ; Un salto de linea al acabar, y silencio
	call imprime_marco		;d42a
	call sonido_reset		;d42d
	ld bc,00000h		;d430   ; Una espera de 65536 antes de irse
L_D433:
	dec bc			;d433
	ld a,b			;d434
	or c			;d435
	jr nz,L_D433		;d436
	pop hl			;d438
	jp L_B68C		;d439   ; Y de aqui, a la pantalla de records
L_D43C:
	pop de			;d43c
	pop bc			;d43d
	ld a,b			;d43e   ; Con el cursor en el primer hueco no hay nada que borrar
	push bc			;d43f
	push de			;d440
	cp 006h		;d441
	jp z,L_D3EC		;d443
	...
	ld b,008h		;d449
L_D44B:
	ld a,000h		;d44b   ; La celda, a cero
	out (098h),a		;d44d
	nop			;d44f
	djnz L_D44B		;d450
	ei			;d452
	ld de,00040h		;d453   ; Una columna atras
	and a			;d456
	sbc hl,de		;d457
	call vram_pon_dir		;d459
	ld b,007h		;d45c
L_D45E:
	ld a,07fh		;d45e   ; Y el cursor ahi, que aqui son OCHO filas: siete del bucle y una mas suelta
	out (098h),a		;d460
	nop			;d462
	djnz L_D45E		;d463
	out (098h),a		;d465
	ei			;d467
	pop de			;d468   ; El hueco vuelve a ser un espacio...
	dec de			;d469
	ld a,020h		;d46a
	ld (de),a			;d46c
	pop bc			;d46d
	inc b			;d46e   ; ...y queda un caracter mas por meter
	jp L_D3EA		;d46f
dir_vram_de_fila_columna:		; De la fila de pixel D y la columna E a la direccion de VRAM: (D>>6)*0x800 + E*0x40 + (D and 0x3F). Identica byte a byte a la de la fase de naves, las 26. Y explica el reparto 56/64/40 de las tres bandas: el buffer empieza en la fila 8, y 8+56=64 y 64+64=128 son las dos fronteras de tercio del SCREEN 2. Las bandas son los tercios de la pantalla, cortados donde manda el hardware
	ld l,000h		;d472   ; La columna se parte en dos: los bits 2-4 al byte alto y los 0-1 arriba del bajo
	ld a,e			;d474
	rra			;d475
	rr l		;d476
	rra			;d478
	...
	and 007h		;d47b
	ld h,a			;d47d
	ld a,d			;d47e   ; La fila dentro del tercio, seis bits, al byte bajo
	and 03fh		;d47f
	or l			;d481
	ld l,a			;d482
	ld a,d			;d483   ; Y el tercio, que son los bits 6-7 de la fila, a los bits 3-4 del alto
	rra			;d484
	rra			;d485
	rra			;d486
	and 018h		;d487
