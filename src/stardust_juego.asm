...
mueve_estrellas:		; Las 48 estrellas de 0xCB09: a cada una le baja una fila por cuadro con vuelta en 0xA0 (el alto del buffer) y la pinta SIEMPRE en su sitio -nunca la mueve-, pero solo si tiene sitio: su propio byte y los siete de debajo a cero o, si ahi hay algo, su propio byte y los siete de encima. Si las dos catas chocan con el decorado, esa estrella se queda sin pintar ese cuadro. El patron es el 0x18 fijo, dos pixeles
	ld de,0cb09h		;c33f   ; Las 48 estrellas, un byte de fila cada una...
	ld c,030h		;c342   ; ...y el indice, que cuenta al reves, de 48 a 1
L_C344:
	ld a,(de)			;c344   ; La fila donde iba esta estrella...
	inc a			;c345
	cp 0a0h		;c346   ; ...una mas abajo, con vuelta al llegar a 160, que es el alto del buffer
	jr c,L_C34C		;c348
	sub 0a0h		;c34a
L_C34C:
	ld (de),a			;c34c   ; La fila nueva se guarda ya, se llegue a pintar o no
	ld h,a			;c34d
	ld l,c			;c34e   ; El indice hace de columna EN BYTES, y va de 1 a 48 cuando el buffer solo mide 24 de ancho: las estrellas con indice de 24 a 47 caen una fila mas abajo de la que dice su byte, y la 48 dos filas
	call buffer_dir		;c34f
	ld (0d3cdh),hl		;c352   ; La direccion se aparta en 0xD3CD, el bloc de dos bytes que comparten esta rutina, repinta_fondo y pinta_sprite
	ld b,008h		;c355   ; Ocho catas hacia abajo, contando la de la propia estrella
L_C357:
	ld a,(hl)			;c357   ; Cada byte del buffer son ocho pixeles de UNA fila, asi que un cero es "aqui no hay nada dibujado"
	and a			;c358
	jr nz,L_C36A		;c359   ; Al primer byte ocupado se deja de mirar hacia abajo y se prueba hacia arriba
	ld a,l			;c35b
	add a,018h		;c35c   ; 24 bytes es una fila entera: las catas van por la misma columna de ocho pixeles, fila a fila
	ld l,a			;c35e
	jp nc,L_C365		;c35f
	inc h			;c362
	jr z,L_C36A		;c363   ; Y si el byte alto da la vuelta, se ha salido del buffer por abajo: tampoco vale
L_C365:
	djnz L_C357		;c365
	jp L_C380		;c367   ; Ocho filas limpias por debajo: se pinta
L_C36A:
	ld hl,(0d3cdh)		;c36a   ; Vuelta al punto de partida...
L_C36D:
	ld a,(hl)			;c36d   ; ...y las mismas ocho catas hacia arriba
	and a			;c36e
	jr nz,L_C385		;c36f
	ld a,l			;c371
	sub 018h		;c372   ; Una fila menos cada vez
	ld l,a			;c374
	jp nc,L_C37E		;c375
	dec h			;c378
	ld a,h			;c379
	cp 040h		;c37a   ; 0x40 es el byte alto donde empieza el buffer: por encima de esa fila ya no hay buffer
	jr c,L_C385		;c37c   ; Con algo ocupado por arriba Y por abajo, esta estrella no se pinta este cuadro
L_C37E:
	djnz L_C36D		;c37e
L_C380:
	ld hl,(0d3cdh)		;c380   ; Y se pinta en SU sitio: 0xD3CD trae la direccion de partida, no la de la ultima cata
	ld (hl),018h		;c383   ; Pinta una estrella: el patron fijo 0x18 (dos pixeles) donde dicta la tabla de las 48 estrellas de 0xCB09
L_C385:
	inc de			;c385
	dec c			;c386   ; A la estrella siguiente
	jr nz,L_C344		;c387
	ret			;c389
repinta_fondo:		; Redibuja las 160 filas del buffer leyendo la fila del mapa de 0x5C32 -seis tiles de 32x32 desde 0x6DE0, 128 B cada uno- y avanza el scroll dos pixeles; al completar los 32 de un tile baja una fila del mapa y da de alta los tiles especiales de la fila nueva. Es la rutina mas cara del bloque: 217.893 muestras en dump/pcs.txt
	ld iy,0cb03h		;c38a   ; IY al bloc del scroll: +0 la fila del mapa, +1 si el scroll esta encendido, +2 el desplazamiento fino, y +3/+4/+5 los tres bytes de trabajo de esta rutina -la tira que toca, cuantas filas de pixel se copian de ella y por donde hay que empezar a leer el tile-
	ld (iy+003h),000h		;c38e   ; A la tira 0, la de arriba
	call entrada_tabla_5c32		;c392   ; IX = 0x5C32 + fila*6: la fila del mapa que asoma por el borde de arriba
	ld de,04000h		;c395   ; Y el destino, el principio del buffer: 24 bytes por 160 filas, de 0x4000 a 0x4EFF
L_C398:
	ld a,(iy+002h)		;c398   ; El desplazamiento fino es lo que manda el reparto en tiras
	cp 020h		;c39b   ; Con el fino a tope las celdas cuadran con el borde de la pantalla: cinco tiras de 32 filas dan las 160 justas y ninguna sale partida
	jr z,L_C3C3		;c39d
	ld a,(iy+003h)		;c39f   ; Con cualquier otro valor hacen falta SEIS tiras, y las de los extremos van cortadas
	and a			;c3a2
	jr z,L_C3B0		;c3a3   ; La 0 es la de arriba, de la que solo se ve la parte de abajo
	cp 005h		;c3a5   ; La 5 es la de abajo, de la que solo se ve la parte de arriba...
	ld a,020h		;c3a7   ; ...32 - fino filas de ella. El `ld a` va delante del `jr` a proposito: cargar no toca las banderas del `cp`
	jr nz,L_C3C3		;c3a9   ; Las tiras 1 a 4 son enteras, 32 filas cada una
	sub (iy+002h)		;c3ab
	jr L_C3C3		;c3ae
L_C3B0:
	ld a,020h		;c3b0   ; La tira de arriba: se ven `fino` filas de pixel, y son las de ABAJO del tile
	sub (iy+002h)		;c3b2
	add a,a			;c3b5   ; Por cuatro, que es lo que ocupa una fila de tile -4 bytes, 32 px-: sale el desplazamiento en BYTES desde el que hay que empezar a leer
	add a,a			;c3b6
	ld (iy+005h),a		;c3b7   ; Ese desplazamiento solo es distinto de cero en esta tira; en las otras cinco el tile se lee desde el principio
	ld a,(iy+002h)		;c3ba   ; Y de la tira 0 se copian justo las filas del fino: fino + 4*32 + (32-fino) = 160, el alto del buffer, salga el fino que salga
	ld (iy+004h),a		;c3bd
	jp L_C3CA		;c3c0
L_C3C3:
	ld (iy+004h),a		;c3c3   ; Las demas tiras: las filas ya vienen en A y el tile se lee desde arriba
	ld (iy+005h),000h		;c3c6
L_C3CA:
	ld a,006h		;c3ca   ; SEIS celdas por tira, que es lo que mide de ancho una fila del mapa: 6 de 32 px son los 192 del buffer
L_C3CC:
	ex af,af'			;c3cc   ; El contador de columnas se aparca en el A alternativo, porque A hace falta entera para la celda
	ld (0d3cdh),de		;c3cd   ; En 0xD3CD, la direccion de esta columna en el buffer: hay que reponerla en cada vuelta porque los `ldi` mueven el destino
	ld d,(ix+000h)		;c3d1   ; El numero de celda entra por el byte ALTO, o sea multiplicado por 256...
	xor a			;c3d4
	ld e,a			;c3d5
	rr d		;c3d6   ; ...y un desplazamiento de 16 bits a la derecha lo deja en celda*128, que es lo que mide un tile
	rr e		;c3d8
	ld hl,06de0h		;c3da   ; El pozo de tiles: 111 dibujos de 32x32 a 128 bytes cada uno, de 0x6DE0 a 0xA55F
	add hl,de			;c3dd
	ld b,(iy+004h)		;c3de   ; Las filas que hay que copiar y desde donde, los dos bytes que dejo el reparto en tiras
	ld e,(iy+005h)		;c3e1
	ld d,a			;c3e4
	add hl,de			;c3e5
	inc ix		;c3e6
	ld de,(0d3cdh)		;c3e8   ; Y el destino, de 0xD3CD
	ld a,d			;c3ec
	cp 050h		;c3ed   ; Pasada la fila 0x5000 ya no hay buffer: esta columna se salta entera
	jr nc,L_C409		;c3ef
	ld c,0ffh		;c3f1   ; C alto a proposito: los `ldi` decrementan BC entero, y arrancando en 0xFF no llegan a robarle una vuelta a B, que es el contador de filas de verdad
L_C3F3:
	...
	ldi		;c3f7
	ldi		;c3f9
	dec b			;c3fb   ; Una fila menos de la tira
	jr z,L_C409		;c3fc
	ld a,e			;c3fe   ; Los cuatro `ldi` ya han adelantado el destino cuatro bytes; con estos veinte mas suman los 24 del ancho del buffer, o sea la fila de abajo. El origen no hay que tocarlo: dentro del tile las filas van seguidas
	add a,014h		;c3ff
	ld e,a			;c401
	jp nc,L_C3F3		;c402   ; Lo normal es que el byte bajo no se lleve nada y la vuelta se de sin tocar D
	inc d			;c405
	jp L_C3F3		;c406
L_C409:
	ex af,af'			;c409   ; Vuelve el contador de columnas del A alternativo
	dec a			;c40a
	jr z,L_C418		;c40b   ; Las seis hechas: se cierra la tira
	ld de,(0d3cdh)		;c40d   ; Del principio de la columna de antes...
	inc de			;c411   ; ...cuatro bytes a la derecha, que es la de al lado
	inc de			;c412
	inc de			;c413
	inc de			;c414
	jp L_C3CC		;c415
L_C418:
	inc (iy+003h)		;c418   ; Una tira mas hecha
	ld a,(iy+002h)		;c41b   ; Cuantas tiras hay depende otra vez del fino...
	cp 020h		;c41e
	ld a,005h		;c420
	adc a,000h		;c422   ; ...y se resuelve con el acarreo del `cp` de arriba, sin un solo salto: cinco con el fino a tope, seis si no
	cp (iy+003h)		;c424   ; Mientras queden tiras, otra vuelta
	jp nz,L_C398		;c427
	ld a,(iy+001h)		;c42a   ; Con el scroll apagado el fondo se redibuja igual, pero no avanza
	and a			;c42d
	ret z			;c42e
	ld a,(iy+002h)		;c42f   ; Completadas las 32 filas de pixel de un tile se baja una fila de mapa...
	cp 020h		;c432
	jr z,L_C43C		;c434
	add a,002h		;c436   ; ...y si no, dos pixeles mas: el fino va 2, 4, 6... hasta 32, o sea DIECISEIS cuadros por fila de mapa
	ld (iy+002h),a		;c438
	ret			;c43b
L_C43C:
	ld a,(iy+000h)		;c43c   ; Con el mapa en la fila 0 se ha llegado al final de la zona
	or a			;c43f
	jp z,L_C476		;c440
	ld (iy+002h),002h		;c443   ; El fino vuelve a 2, no a 0: el cuadro en el que se cambia de fila ya cuenta como el primero
	dec (iy+000h)		;c447   ; Una fila menos de mapa, que es hacia donde avanza la camara
	call entrada_tabla_5c32		;c44a   ; La fila NUEVA, la que acaba de asomar por arriba
	push ix		;c44d
	pop hl			;c44f
	ld b,006h		;c450   ; Sus seis celdas, una a una
L_C452:
	ld a,(hl)			;c452   ; De cada celda solo importa si es de las que tienen vida propia...
	cp 05dh		;c453   ; ...la 0x5D lo es...
	jr z,L_C45D		;c455
	cp 047h		;c457   ; ...de la 0x47 en adelante no...
	jr nc,L_C460		;c459
	cp 030h		;c45b   ; ...y tampoco por debajo de la 0x30: las que se dan de alta al asomar son la 0x5D y las de 0x30 a 0x46
L_C45D:
...
entrada_tabla_5c32:		; IX = 0x5C32 + (iy+000)*6: la entrada de la tabla de 85 huecos de 6 bytes que el arranque limpia con el ldir de 0xBDC2
	ld ix,05c32h		;c464   ; por seis: las entradas de la tabla de 0x5C32 son de seis bytes
	ld l,(iy+000h)		;c468
	ld h,000h		;c46b
	ld d,h			;c46d
	ld e,l			;c46e
	add hl,hl			;c46f   ; por tres y luego por dos, que es por seis
	add hl,de			;c470
	add hl,hl			;c471
	ex de,hl			;c472
	add ix,de		;c473
	ret			;c475
L_C476:
	xor a			;c476   ; Fin de la zona: 0xCB39 a cero es lo que mira el bucle de partida para saber que se ha acabado
	ld (0cb39h),a		;c477
	ret			;c47a
...
L_C4B9:
	...
	inc hl			;c4bf
	ld e,(hl)			;c4c0
	scf			;c4c1   ; ...y el `scf` pone el primer uno que va a entrar por la derecha, porque ni el `inc hl` ni el `ex de,hl` tocan el acarreo
	ex de,hl			;c4c2
L_C4C3:
...
tira_mascara24:		; La tira de la mascara: siete `adc hl,hl / adc a,a` seguidos, en la que pinta_sprite entra por el peldano n-1 para dar 8-n pasos. Entra con A=0xFF y acarreo, o sea rellenando de unos
	...
	adc a,a			;c4ce
	adc hl,hl		;c4cf   ; Se entra por el peldano que deja 8-(X and 7) pasos, con lo que el sprite acaba a (X and 7) pixeles del borde izquierdo de la ventana, que es justo donde se le quiere. Los otros tres desplazadores del bloque son esta misma tira con el relleno o el ancho cambiados
	adc a,a			;c4d1
	adc hl,hl		;c4d2
	adc a,a			;c4d4
	adc hl,hl		;c4d5
	...
	adc a,a			;c4e0
L_C4E1:
	ex de,hl			;c4e1   ; Las dos mitades bajas de la ventana pasan a DE...
	pop hl			;c4e2   ; ...y HL vuelve a ser el destino en el buffer
	and (hl)			;c4e3   ; Los sprites, con mascara: el and abre el hueco...
	ld (hl),a			;c4e4   ; El byte de la izquierda de la ventana, el primero de los tres del ancho de 24
	inc hl			;c4e5   ; Tres bytes seguidos, 24 pixeles, para un sprite de 16: los 8 de sobra son el margen que hace falta para poder colocarlo en cualquier X
	ld a,d			;c4e6
	and (hl)			;c4e7
	ld (hl),a			;c4e8
	inc hl			;c4e9
	...
	inc de			;c4ee   ; ...dos bytes mas alla, que es donde empieza el DIBUJO de esta misma fila
	inc de			;c4ef
	...
	ex de,hl			;c4f9
L_C4FA:
	jr L_C4FA		;c4fa   ; El otro `jr` parcheado, con el mismo numero que el de la mascara
atajo_dibujo24:		; Lo mismo que atajo_mascara24 para el dibujo: A = H, H = L, L = 0x00, rellenando de ceros
	ld a,h			;c4fc   ; El atajo gemelo, cambiando los unos por ceros
	ld h,l			;c4fd
	ld l,000h		;c4fe
	jp L_C518		;c500
tira_dibujo24:		; La tira del dibujo: la misma de siete peldanos, byte a byte igual que la de la mascara. Se entra con `xor a`, que pone A a cero Y limpia el acarreo, asi que rellena de ceros
	adc hl,hl		;c503   ; La tira gemela: los mismos siete peldanos, y lo unico distinto es con que se entra y que abajo se estampa con `or`
	adc a,a			;c505
	adc hl,hl		;c506   ; Siete peldanos como la de la mascara, ni uno mas: 8 es el maximo desplazamiento y ese se despacha con el atajo de arriba
	adc a,a			;c508
	adc hl,hl		;c509
	adc a,a			;c50b
	adc hl,hl		;c50c
	...
	adc a,a			;c517
L_C518:
	ex de,hl			;c518   ; Las dos mitades bajas, igual que en la mascara
	pop hl			;c519
	or (hl)			;c51a   ; `or` en vez de `and`: el dibujo enciende lo blanco del sprite. En 1.283 de las 1.328 filas del pozo el dibujo cae DENTRO de la mascara, o sea que no pisa lo que la mascara acaba de dejar en negro; las 45 que no son de cinco dibujos tramados (0x18 y 0x4B-0x4E), donde el negro y el blanco caen en el mismo pixel y gana el blanco porque el `or` va detras
...
L_C525:
	...
	xor a			;c532
L_C533:
	sub 00fh		;c533   ; Si no se ha pasado, este `sub` deshace el `add` de arriba y H sale como entro: la comprobacion no cuesta ni una instruccion de mas
	ld h,a			;c535
...
buffer_dir:		; La direccion en el buffer de pantalla: HL = 0x4000 + fila*24 + columna, con el *24 hecho a mano (x3 y tres veces x2)
	...
	ld a,l			;c542   ; L trae la columna y H la fila; se cambian de sitio para poder multiplicar la fila
...
pinta_glifo:		; Estampa en el buffer el glifo A de la fuente de 0xCA93 (16 B por glifo, 8 filas de mascara y dibujo) en la posicion HL, con el desplazamiento fino parcheando los `jr` de 0xC58E y 0xC5B3
	...
	ld l,a			;c556
	add hl,hl			;c557   ; Cuatro veces por dos: 16 bytes por glifo, que son 8 filas de un byte de mascara y otro de dibujo
	add hl,hl			;c558
	add hl,hl			;c559
	add hl,hl			;c55a
	...
	and 007h		;c568   ; Y el mismo desplazamiento fino, pero la tira de 16 bits gasta DOS bytes por peldano en vez de tres, asi que el numero es otro
...
L_C57E:
	...
	jr L_C5D2		;c585
L_C587:
	push hl			;c587   ; El destino a la pila, que HL va a hacer de ventana
...
tira_mascara16:		; La tira de la mascara de 16 bits: siete `adc hl,hl`, sin A porque el relleno de unos ya viene en H
	adc hl,hl		;c596   ; La ventana cabe entera en HL, asi que aqui sobra el `adc a,a` que la de 24 bits necesitaba para arrastrar su tercer byte
	adc hl,hl		;c598
	adc hl,hl		;c59a
	adc hl,hl		;c59c
	adc hl,hl		;c59e
...
L_C5A4:
	...
	ld c,l			;c5a5
	pop hl			;c5a6   ; Y vuelve el destino
	and (hl)			;c5a7
	ld (hl),a			;c5a8   ; Dos bytes, 16 pixeles, para un glifo de 8: el mismo margen y el mismo `and` que en los sprites
	inc hl			;c5a9
	ld a,c			;c5aa
	and (hl)			;c5ab
	ld (hl),a			;c5ac
	push hl			;c5ad   ; El destino otra vez a la pila, que hace falta para el dibujo
	xor a			;c5ae   ; El dibujo, con H a cero y sin acarreo -el `xor a` limpia las dos cosas-: rellena de CEROS para no pintar de mas
	ld h,a			;c5af
	ld a,(de)			;c5b0   ; El segundo byte de la fila es el dibujo
	inc de			;c5b1
	ld l,a			;c5b2
L_C5B3:
	jr L_C5B3		;c5b3   ; El `jr` gemelo, con el mismo numero
atajo_dibujo16:		; Desplazamiento de ocho del dibujo de 16 bits: H = L, L = 0x00
	ld h,l			;c5b5   ; El atajo de ocho del dibujo, con cero de relleno
	ld l,000h		;c5b6
	jp L_C5C9		;c5b8
tira_dibujo16:		; La tira del dibujo de 16 bits, los siete peldanos: tres de ellos figuraban como relleno porque el punto medido (0xC5C1) caia en el cuarto
	adc hl,hl		;c5bb   ; Siete `adc hl,hl` y a estampar con `or`: la cuarta y ultima de las tiras del bloque, todas cortadas por el mismo patron
	adc hl,hl		;c5bd   ; El relleno de ceros lo trae ya puesto H, asi que el acarreo con el que se entra tiene que estar limpio
	adc hl,hl		;c5bf
	adc hl,hl		;c5c1
	adc hl,hl		;c5c3
	adc hl,hl		;c5c5
...
L_C5C9:
	...
	ld c,h			;c5ca
	pop hl			;c5cb
	or (hl)			;c5cc   ; ...estampados con `or`, y de derecha a izquierda: escribiendo primero el byte de la derecha y bajando con un `dec hl` se ahorra el `push` que haria falta para volver al principio
	ld (hl),a			;c5cd
	dec hl			;c5ce
	ld a,c			;c5cf
	or (hl)			;c5d0
...
L_C5D2:
	...
	add a,018h		;c5d3
	ld l,a			;c5d5
	jr nc,L_C5E0		;c5d6   ; Lo normal es que no se lleve nada y no haya que tocar H
	inc h			;c5d8
	ld a,h			;c5d9
...
L_C5E9:
	ld (hl),e			;c5e9   ; dos bytes por vuelta: la palabra que se siembra en el buffer
	inc hl			;c5ea
	ld (hl),d			;c5eb
	inc hl			;c5ec
	djnz L_C5E9		;c5ed
...
pinta_rejilla:		; Estampa en el buffer las 25 celdas de la rejilla de 0xC920, tres bytes por 24 filas cada una, con tres dibujos segun el valor: el 1 alterna dos bytes, el 2 saca ocho filas de la fuente de 0xCA84 y del 3 en adelante lee 0x69A8 + (celda-3)*72, que es la tabla de los centinelas. No la llama nadie: se entra por el `jp` de 0xD8A0
	ld a,h			;c69f   ; Quita el sesgo de 0x20 con que se llevan las filas
	sub 020h		;c6a0
	ld h,a			;c6a2
	ld a,000h		;c6a3   ; D entra en 0x00 o en 0xFF -el `sbc a,a` de 0xD896-, o sea 0 o -1; sumarle el prestamo es restar 0x20 a la fila de nueve bits entera, y vale porque la cuenta va modulo 512 filas
	adc a,d			;c6a5
	ld d,a			;c6a6
	call buffer_dir		;c6a7   ; La direccion en el buffer, con la fila recortada a ocho bits
	ld a,d			;c6aa   ; Si el noveno bit no ha sobrevivido, esa direccion ya vale
	and a			;c6ab
	jr z,L_C6B2		;c6ac
	ld a,h			;c6ae   ; Y si ha sobrevivido, 0x1800 arriba: 256 filas de 24 bytes, que en el anillo de 0x3000 es tanto subirlas como bajarlas
	add a,018h		;c6af
	ld h,a			;c6b1
L_C6B2:
	exx			;c6b2   ; De aqui en adelante el destino se queda en un juego de registros y los dos contadores en el otro
	ld a,06fh		;c6b3   ; El tope del anillo aparcado en A', y dentro de la rutina no lo lee nadie
	ex af,af'			;c6b5
	ld c,005h		;c6b6   ; Cinco filas de celdas...
L_C6B8:
	ld b,005h		;c6b8   ; ...y cinco columnas en cada una
L_C6BA:
	ld a,(ix+000h)		;c6ba   ; El valor de la celda, que es a la vez el dibujo y el estado de lo que hay en ella
	inc ix		;c6bd
	and a			;c6bf
	exx			;c6c0
	jp z,L_C705		;c6c1   ; La celda 0 esta vacia: ni se pinta ni se borra
	cp 003h		;c6c4   ; De 3 en adelante es un centinela de la tabla de 0x69A8
	jp nc,celda_centinela		;c6c6
	dec a			;c6c9   ; La celda 1 es la tira de dos bytes...
	jp z,celda_uno		;c6ca
	push hl			;c6cd   ; ...y la 2, la que cae por aqui, ocho filas de la fuente de 0xCA84
	ld de,000c0h		;c6ce   ; 0xC0 son ocho filas de 24: el dibujo de la celda 2 no empieza arriba del todo, sino a un tercio de la celda
	add hl,de			;c6d1
	ld a,06fh		;c6d2   ; El tope del anillo, la primera de las seis veces
	cp h			;c6d4
	jr nc,L_C6D9		;c6d5
	ld h,040h		;c6d7
L_C6D9:
	ld bc,00816h		;c6d9   ; Ocho filas, y 0x16 = 22 para completar el paso de fila con los dos `inc hl` que quedan
	ld de,0ca84h		;c6dc   ; La fuente de la celda 2, en 0xCA84: los cuatro patrones 00/FF/55/AA
L_C6DF:
	ld a,04fh		;c6df   ; Solo se pinta lo que cae dentro del buffer; de 0x5000 arriba es anillo y ahi no hay pantalla
	cp h			;c6e1
	jr nc,L_C6E8		;c6e2
	inc hl			;c6e4   ; Fuera del buffer no se pinta pero HL avanza lo mismo, y por eso los dos `inc hl` estan en los dos caminos
	inc hl			;c6e5
	jr L_C6EE		;c6e6
L_C6E8:
	ld a,(de)			;c6e8   ; El mismo byte en las tres columnas de la celda
	ld (hl),a			;c6e9
	inc hl			;c6ea
	ld (hl),a			;c6eb
	inc hl			;c6ec
	ld (hl),a			;c6ed
L_C6EE:
	ld a,l			;c6ee   ; El paso de fila, sumado a ocho bits para no pagar el `add hl,de`
	add a,c			;c6ef
	ld l,a			;c6f0
	jp nc,L_C701		;c6f1
	inc h			;c6f4
...
L_C701:
	...
	pop hl			;c704
L_C705:
	inc hl			;c705   ; Tres bytes a la derecha: la celda de al lado
	inc hl			;c706
	inc hl			;c707
	exx			;c708
	djnz L_C6BA		;c709   ; Las cinco columnas
	exx			;c70b
	ld de,00231h		;c70c   ; 0x231 son las 24 filas de la celda -0x240- menos los quince bytes que ya han adelantado las cinco columnas
	add hl,de			;c70f
	ld a,050h		;c710   ; Y el tope del anillo otra vez, ahora entre fila y fila de celdas
	cp h			;c712
	jr nc,L_C71E		;c713
	ld a,06fh		;c715
	cp h			;c717
...
L_C71E:
	exx			;c71e
	dec c			;c71f   ; Las cinco filas
	jp nz,L_C6B8		;c720
	ret			;c723
celda_uno:		; El dibujo de la celda 1: una tira de UN byte de ancho -el del medio de los tres- y 24 filas de alto, alternando 0x4E y 0x56
	push hl			;c724
	inc hl			;c725   ; El byte del medio de los tres, que deja la tira centrada en la celda
	ld de,00018h		;c726
	ld bc,00c56h		;c729   ; Doce vueltas de dos filas, que son las 24 de la celda; C es el byte de las filas pares
L_C72C:
	ld a,04fh		;c72c   ; Fuera del buffer no se pinta
	cp h			;c72e
	jr c,L_C733		;c72f
...
L_C74E:
	djnz L_C72C		;c74e
	pop hl			;c750
	jp L_C705		;c751   ; Y a la celda de al lado por el camino comun
celda_centinela:		; Las celdas de 3 en adelante: los 72 bytes de 0x69A8 + (celda-3)*72 copiados con `ldi`, que son las 24 filas de 3 de un centinela
	push hl			;c754
	push hl			;c755
	sub 003h		;c756   ; El indice dentro de la tabla de los centinelas
	ld h,000h		;c758
	ld l,a			;c75a
	ld d,h			;c75b
	ld e,l			;c75c
	add hl,hl			;c75d   ; Por 3, por 9 y tres veces por 2: 72, los bytes que ocupa un centinela
	add hl,de			;c75e
	ld d,h			;c75f
	ld e,l			;c760
	add hl,hl			;c761
	...
	add hl,de			;c769
	pop de			;c76a
	ld bc,018ffh		;c76b   ; B son las 24 filas; C entra en 0xFF para que los `ldi` -que bajan BC entero- no lleguen a robarle una vuelta al `djnz`, que 72 restas no le hacen mella
L_C76E:
	ld a,d			;c76e   ; Solo se copia dentro del buffer
	cp 04fh		;c76f
	jr c,L_C77A		;c771
	inc hl			;c773   ; Fuera se salta la fila pero la fuente avanza igual, y el paso va entero porque aqui los `ldi` no han adelantado nada
	inc hl			;c774
	inc hl			;c775
	ld a,018h		;c776
	jr L_C782		;c778
L_C77A:
	...
	ldi		;c77c
	ldi		;c77e
	ld a,015h		;c780   ; 0x15 son 21, porque los tres `ldi` ya han adelantado 3 de las 24
L_C782:
	add a,e			;c782   ; El paso de fila del destino, tambien a ocho bits
	ld e,a			;c783
	jr nc,L_C78E		;c784
	inc d			;c786
	ld a,06fh		;c787   ; El tope del anillo, la ultima de las seis veces
	cp d			;c789
	jr nc,L_C78E		;c78a
	ld d,040h		;c78c
L_C78E:
	djnz L_C76E		;c78e   ; Las 24 filas del centinela
	pop hl			;c790
	jp L_C705		;c791
...
rumbo_suma_ejes:		; Suma el paso a la posicion, cada eje por su lado y a ocho bits
	...
	ld l,a			;c801
	ret			;c802
rumbo_hacia:		; La mascara de rumbo de (B,C) -el que persigue- hacia (D,E) -el objetivo-, con la fila en B y D y la columna en C y E. Reparto de bits el mismo del mando y de poda_rumbo_nave: bit 0 bajar, bit 1 subir, bit 2 a la derecha y bit 3 a la izquierda. Cada componente solo cuenta si dos veces y media ella misma llega a la otra, o sea que la diagonal se coge con cualquier proporcion entre 1:2,5 y 2,5:1. Nunca vuelve a cero: si una componente no llega, la otra pasa seguro. Vuelve en A y en C
	ld a,b			;c803   ; La fila del que persigue contra la del objetivo
	sub d			;c804
	jr c,L_C80C		;c805
	ld h,002h		;c807   ; El objetivo esta mas arriba: el bit 1, subir
	jp L_C810		;c809
L_C80C:
	ld h,001h		;c80c   ; Y aqui mas abajo: el bit 0, bajar, y la diferencia cambiada de signo
	neg		;c80e
L_C810:
	ld d,a			;c810   ; D se queda con la diferencia de filas en valor absoluto
	ld a,c			;c811   ; Y ahora la columna
	sub e			;c812
	jr c,L_C81A		;c813
	ld l,008h		;c815   ; El objetivo esta a la izquierda: el bit 3
	jp L_C81E		;c817
L_C81A:
	ld l,004h		;c81a   ; Y aqui a la derecha: el bit 2
	neg		;c81c
L_C81E:
	ld e,a			;c81e   ; E, la diferencia de columnas en valor absoluto
	ld b,a			;c81f   ; Dos veces y media esa diferencia, en tres pasos: la mitad aparte, el doble, y una suma
	srl b		;c820
	add a,a			;c822
	jr c,L_C82D		;c823   ; Si el doble ya no cabe en ocho bits, la lateral es enorme al lado de la otra y cuenta seguro
	add a,b			;c825
	jr c,L_C82D		;c826
	cp d			;c828   ; Contra la diferencia de filas: la componente lateral solo entra en la mascara si dos veces y media ella misma llega a la vertical
	ld c,000h		;c829   ; Si no llega, la mascara se queda sin bit de columna...
	jr c,L_C82E		;c82b
L_C82D:
	ld c,l			;c82d   ; ...y si llega, entra el que toque
L_C82E:
	ld a,d			;c82e   ; Lo mismo con la vertical, y esta vez contra la diferencia de columnas
	ld b,a			;c82f
	srl b		;c830
	add a,a			;c832
	jr c,L_C83B		;c833
	...
	cp e			;c838
	ld a,c			;c839
	ret c			;c83a   ; No llega: vuelve con la columna sola, un rumbo horizontal puro
L_C83B:
	ld a,c			;c83b
	or h			;c83c   ; Llegan las dos y sale una diagonal. Nunca puede salir cero: si una componente no llega, la otra pasa seguro
	ld c,a			;c83d
	ret			;c83e
...
alta_bandada:		; Mete un objeto en la tabla de 0xC97B (contador 0xC97A, tope 4, entradas de 5 B) con rumbo inicial 0x14 y el A' de entrada como RETARDO en (ix+004): si es cero suena 0xEB42 al nacer. El OTRO valor que el juego pasa, 0x1B desde tile_32, no llega a sonar nunca: el sonido de mueve_bandada (0xC24B) pide que un dec deje el retardo en cero justo, y de 0x0F para abajo baja de tres en tres -15, 12, 9, 6, 3, 0- sin pasar por el 1. Vuelve con carry si ha entrado
	ld hl,0c97ah		;c882   ; El contador de la tabla
	ld a,(hl)			;c885
	cp 004h		;c886   ; Cuatro a la vez como mucho
	ret nc			;c888
	inc (hl)			;c889   ; Sube el contador, y el valor viejo es el hueco que toca
	ld h,000h		;c88a   ; El indice, a HL
	ld l,a			;c88c
	ld d,h			;c88d
	ld e,l			;c88e
	add hl,hl			;c88f   ; Por cuatro y una vez mas: entradas de cinco bytes
	add hl,hl			;c890
	add hl,de			;c891
	ld de,0c97bh		;c892   ; La base de la tabla, que empieza justo detras del contador
	add hl,de			;c895
	ld (hl),c			;c896   ; Columna y fila
	inc hl			;c897
	ld (hl),b			;c898
	inc hl			;c899
	ld (hl),014h		;c89a   ; El rumbo de nacimiento: 0x14 son el rumbo 4 -bajar- con el contador de giro de gira_rumbo ya en 0x10
	inc hl			;c89c
	ld (hl),000h		;c89d   ; El contador de cuadros a cero, que es lo que (ix+003) significa mientras el enemigo esta vivo
	inc hl			;c89f
	ex af,af'			;c8a0   ; Y el retardo de entrada, que viene en A'
	ld (hl),a			;c8a1
	and a			;c8a2   ; Si nace a cero, la entrada suena ya
	scf			;c8a3   ; El `scf` esta antes del `ret nz` a proposito: el que entra vuelve con carry por los dos caminos
	ret nz			;c8a4
	ld de,0eb42h		;c8a5   ; El mismo guion que suena cuando mueve_bandada agota un retardo
...
alta_disparo:		; Mete un disparo en la tabla de 0xC999 (contador en 0xC998, entradas de 5 B) si no hay ya dos en el aire, y arranca el sonido 0xEA81 en el canal 1. Vuelve con carry si ha salido
	ld hl,0c998h		;c8ad   ; El contador
	ld a,(hl)			;c8b0
	cp 002h		;c8b1   ; Dos disparos en el aire como mucho
	ret nc			;c8b3
	inc (hl)			;c8b4
	ld h,000h		;c8b5
	ld l,a			;c8b7
	...
	add hl,hl			;c8bb
	add hl,de			;c8bc
	ld de,0c999h		;c8bd   ; La base, detras del contador
	add hl,de			;c8c0
	ld (hl),000h		;c8c1   ; Los cinco bytes: primero un cero y el A' de entrada, que el que la mueve (0xDF0F) recoge en C y en B...
	inc hl			;c8c3
	ex af,af'			;c8c4
	ld (hl),a			;c8c5
	inc hl			;c8c6
	ld (hl),c			;c8c7   ; ...luego la columna y la fila, que son las que recoge en DE...
	inc hl			;c8c8
	ld (hl),b			;c8c9
	inc hl			;c8ca
	ld (hl),000h		;c8cb   ; ...y un quinto a cero, el que 0xDF48 compara contra 0xB4
	ld a,001h		;c8cd   ; Canal 1
	ld de,0ea81h		;c8cf
	call arranca_guion_libre		;c8d2
	scf			;c8d5   ; Y vuelve con carry, que ha salido
	ret			;c8d6
...
; ----------------------------------------------------------------------
; DATOS fotograma_y_rumbo_de_la_nave: El fotograma de la nave en los cinco
;   bits bajos y su rumbo en los tres altos (1 B): lo lleva actualiza_nave
;   0xc9a3..0xc9a4  (1 bytes)
DATA_fotograma_y_rumbo_de_la_nave:
	defb 000h	; c9a3
...
;   arriba-izquierda (0x0A). La lee rumbo_a_mascara (0xC7CD)
;   0xc9a4..0xc9ac  (8 bytes)
...
;   cuatro bits (16 B). La lee rumbo_a_mascara2 (0xC7D3). Las mascaras que se
;   contradicen NO son un descuido: el eje que se anula se cae y queda el otro
;   -0x07 (arriba+abajo+derecha) da 2, que es derecha, y 0x0E
;   (arriba+derecha+izquierda) da 0, que es arriba-, y solo las cuatro en las
;   que no sobrevive nada -0x00, 0x03, 0x0C y 0x0F- valen 0xFF
;   0xc9ac..0xc9bc  (16 bytes)
...
recorre_tiles_especiales:		; Recorre las entradas de 0xCB3A, sube (ix+000) una por cuadro y salta a la rutina de (ix+003/004) con el `jp (hl)` de 0xCB99; al llegar (ix+000) a 0x60 la da de baja en vez de gobernarla
	...
	ld b,a			;cb83
L_CB84:
	push bc			;cb84   ; El contador de vueltas, a la pila: la rutina de la ficha se lleva por delante los registros
	inc (ix+000h)		;cb85   ; Un cuadro mas de vida para este tile especial
	ld a,(ix+000h)		;cb88
	cp 060h		;cb8b   ; A los 0x60 cuadros se le da de baja y no se le gobierna mas
	jr z,L_CB9A		;cb8d
	ld hl,lcb9dh		;cb8f   ; La vuelta se apila a mano y se salta con `jp (hl)`: asi la rutina de la ficha vuelve sola a 0xCB9D, sin que este bucle gaste un `call`
	push hl			;cb92
	ld l,(ix+003h)		;cb93   ; Cada ficha lleva en el +3/+4 la direccion de quien la gobierna
	ld h,(ix+004h)		;cb96
	jp (hl)			;cb99
L_CB9A:
	call baja_tile_especial		;cb9a
L_CB9D:
	ld de,00008h		;cb9d   ; Ocho bytes por ficha
	add ix,de		;cba0
	pop bc			;cba2
	djnz L_CB84		;cba3
	ret			;cba5
persigue_con_velocidad:		; Corrige la velocidad de BC hacia el objetivo DE un punto por eje, la mitad de las veces y saturada a mas menos cinco, aplica el movimiento y, si la columna se pasa de 0xB8, la repone e invierte el sentido. La usa mueve_disparos
	ld (0d3cdh),hl		;cba6   ; La posicion de partida, guardada por si hay que rebotar
	call azar		;cba9   ; Solo una de cada dos veces se corrige el rumbo: asi el tiro conserva inercia y describe una curva en vez de una linea quebrada
	and 001h		;cbac
	jr z,L_CBDC		;cbae
	ld a,h			;cbb0   ; La fila del tiro contra la del blanco...
	cp d			;cbb1
	ld a,b			;cbb2
	jr nc,L_CBBD		;cbb3
	add a,005h		;cbb5   ; ...y la velocidad vertical sube un paso, saturando en +5
	cp 00ah		;cbb7
	adc a,000h		;cbb9
	jr L_CBC3		;cbbb
L_CBBD:
	add a,005h		;cbbd   ; O baja uno, saturando en -5. El sesgo de cinco es lo que permite hacer las dos cosas con comparaciones sin signo
	cp 001h		;cbbf
	adc a,0ffh		;cbc1
L_CBC3:
	sub 005h		;cbc3   ; Quitado el sesgo, la velocidad vuelve a su rango de -5 a +5
	ld b,a			;cbc5
	ld a,l			;cbc6   ; Y lo mismo con la columna
	cp e			;cbc7
	ld a,c			;cbc8
	jr nc,L_CBD3		;cbc9
	add a,005h		;cbcb
...
L_CBD9:
	...
	ld c,a			;cbdb
L_CBDC:
	add hl,bc			;cbdc   ; El paso, sea el de siempre o el recien corregido
	ld a,l			;cbdd   ; El recorte, solo en la columna: 0xB8 de ancho util, cuatro MENOS que el 0xBC con el que poda mueve_tabla
	cp 0b8h		;cbde
	jr c,L_CBEA		;cbe0
	ld a,(0d3cdh)		;cbe2   ; Se repone la X de partida y se niega la velocidad horizontal: el tiro rebota contra el lado en vez de morir
	ld l,a			;cbe5
	ld a,c			;cbe6
	neg		;cbe7
	ld c,a			;cbe9
...
recorre_las_dos_tablas:		; Mueve las dos tablas seguidas, parcheando antes la velocidad de cada una: 6 para la de 0xC93A (seis huecos) y 9 para la de 0xC953 (nueve)
	ld ix,lcc32h		;cbeb   ; Las dos tablas de disparos pasan por el mismo motor, y lo unico que cambia es el paso: se parchea EN CALIENTE el operando del `ld bc` de 0xCC32
	ld (ix+001h),006h		;cbef   ; Seis por eje para los tiros enemigos...
	ld (ix+002h),006h		;cbf3
	ld ix,0c93ah		;cbf7   ; ...sobre la tabla de 0xC93A, que acaba en 0xC952, y con el 5 por identificador
	ld hl,0c952h		;cbfb
	ld a,005h		;cbfe
	call mueve_tabla		;cc00
	ld ix,lcc32h		;cc03
	ld (ix+001h),009h		;cc07   ; Nueve por eje para los disparos del jugador, que van mas rapidos...
	ld (ix+002h),009h		;cc0b
	ld ix,0c953h		;cc0f   ; ...sobre la de 0xC953, que acaba en 0xC977, con el 4 por identificador
	ld hl,0c977h		;cc13
	ld a,004h		;cc16
mueve_tabla:		; Recorre una tabla de objetos cualquiera -IX en la primera entrada, contador en (ix-001), entradas de 4 B-, les aplica el rumbo con el paso que le hayan parcheado en 0xCC32, los pinta con pinta_glifo y retira los que pasan de la fila 0xC0 o de la columna 0xBC
	ld (0c977h),hl		;cc18   ; El motor comun aparca en 0xC977 donde acaba la tabla que va a recorrer y en 0xC979 cual de las dos es, que es lo que la baja necesita para compactar
	ld (0c979h),a		;cc1b
	ld a,(ix-001h)		;cc1e   ; El contador vive UN byte por delante de la tabla, de ahi el indice negativo
	and a			;cc21
	ret z			;cc22   ; Sin ninguno vivo no hay nada que recorrer
	ld b,a			;cc23
L_CC24:
	push bc			;cc24   ; El contador de vueltas se aparca, que la baja lo va a mirar en la pila
	ld l,(ix+000h)		;cc25
	ld h,(ix+001h)		;cc28
	ld a,(ix+002h)		;cc2b   ; El rumbo a 0x80 exacto es la marca de muerto, que pone quien acierta el impacto
	cp 080h		;cc2e
	jr z,L_CC56		;cc30
L_CC32:
	ld bc,00707h		;cc32   ; Aqui esta el operando que se parchea. El 0x0707 que trae el binario no lo escribe nadie: los dos llamantes lo pisan antes de entrar
	call aplica_rumbo		;cc35
	ld a,h			;cc38   ; Fuera por abajo...
	cp 0c0h		;cc39
	jr nc,L_CC56		;cc3b
	ld a,l			;cc3d   ; ...o por la derecha, y en los dos casos se da de baja
	cp 0bch		;cc3e
	jr nc,L_CC56		;cc40
	ld (ix+000h),l		;cc42   ; La posicion nueva, ya buena, vuelve a la ficha
	ld (ix+001h),h		;cc45
	ld a,002h		;cc48   ; Los dos disparos se pintan con el mismo glifo, el 2 de la fuente de 0xCA93
	call pinta_glifo		;cc4a
	ld de,00004h		;cc4d   ; Cuatro bytes por ficha, y a la siguiente
	add ix,de		;cc50
L_CC52:
	pop bc			;cc52
	djnz L_CC24		;cc53
	ret			;cc55
L_CC56:
	ld a,(0c979h)		;cc56   ; La baja: primero, cual de los dos contadores hay que bajar
	cp 005h		;cc59
	ld hl,0c939h		;cc5b
	jr z,L_CC63		;cc5e
	ld hl,0c952h		;cc60
L_CC63:
	dec (hl)			;cc63   ; Una ficha menos
	pop bc			;cc64
	push bc			;cc65
	ld a,b			;cc66   ; Si la que muere es la ultima que quedaba por mirar, no hay nada que mover
	cp 001h		;cc67
	jp z,L_CC52		;cc69
	push ix		;cc6c   ; El compactado: se trae encima todo lo que va detras de la ficha muerta...
	pop de			;cc6e
	push de			;cc6f
	inc de			;cc70
	inc de			;cc71
	inc de			;cc72
	inc de			;cc73
	ld hl,(0c977h)		;cc74   ; ...y lo que se mueve es lo que va de la ficha siguiente al final de la tabla, que es el 0xC977 que se aparco al entrar
	and a			;cc77
	sbc hl,de		;cc78
	ld b,h			;cc7a
	ld c,l			;cc7b
	ex de,hl			;cc7c
	pop de			;cc7d
	ldir		;cc7e   ; Un `ldir` y hueco cerrado. IX no avanza, asi que la ficha que ocupa el sitio se mira en la vuelta siguiente y no se salta ninguna
	jp L_CC52		;cc80
alta_enemigo:		; Intenta meter un objeto en la tabla de 0xC939 (tope 6): solo lo hace si el azar supera (7 - zona)*16, que es como sube la dificultad, y suena 0xEA44 al nacer
	ld hl,0c939h		;cc83   ; El contador de los tiros enemigos
	ld a,(0e157h)		;cc86   ; La zona, de 1 a 7
	ld e,a			;cc89
	ld a,007h		;cc8a   ; (7 - zona) por 16: el liston baja de 96 en la zona 1 a CERO en la 7, donde ya pasa siempre, y por eso salen mas enemigos segun avanza la partida
	sub e			;cc8c
	add a,a			;cc8d
	add a,a			;cc8e
	add a,a			;cc8f
	add a,a			;cc90
	ld e,a			;cc91
	call azar		;cc92   ; La tirada
	cp e			;cc95
	ccf			;cc96   ; El `ccf` hace dos cosas de una vez: da la vuelta a la comparacion -se sigue cuando la tirada LLEGA al liston- y deja el acarreo a cero en el `ret`, que es el "no ha entrado" que espera el llamador
	ret nc			;cc97
	ld e,006h		;cc98   ; Seis a la vez como mucho
	exx			;cc9a   ; Del `exx` de aqui al de 0xCCAE no hay ninguno mas: el puntero al contador y el tope se quedan en un juego de registros y el canal, el guion y la posicion en el otro
	ld h,001h		;cc9b   ; Canal 1 para los tiros enemigos...
	ld de,0ea44h		;cc9d
...
alta_objeto_c952:		; Mete un objeto en la tabla de 0xC952 (tope 9) con el sonido 0xEA38, sin tirada de azar previa
	...
	ld e,009h		;cca6
	exx			;cca8
	ld h,002h		;cca9   ; ...y canal 2 para los del jugador
	ld de,0ea38h		;ccab
alta_en_tabla:		; El alta comun a las tres tablas: si el contador no ha llegado al tope, lo incrementa, arranca el sonido DE en el canal H y escribe C, B y A' en la entrada indice*4. Vuelve con carry si ha entrado
	exx			;ccae   ; Vuelve al juego de registros donde estan el contador y el tope
	ld a,(hl)			;ccaf
	cp e			;ccb0   ; Llena: no entra
	ret nc			;ccb1   ; Llena: vuelve sin carry, y en el mismo juego de registros con el que la llamaron
	exx			;ccb2
	push af			;ccb3   ; El contador es el indice del hueco y arranca_guion_libre se come A, asi que se guarda
	ld a,h			;ccb4
	call arranca_guion_libre		;ccb5
	pop af			;ccb8
	exx			;ccb9
	inc (hl)			;ccba   ; Una ficha mas
	ld e,l			;ccbb   ; La tabla empieza justo detras del contador
	ld d,h			;ccbc
	inc de			;ccbd
	ld h,000h		;ccbe   ; Por cuatro: entradas de cuatro bytes...
	ld l,a			;ccc0
	add hl,hl			;ccc1   ; Por cuatro: entradas de cuatro bytes...
	add hl,hl			;ccc2
	add hl,de			;ccc3
	ld (hl),c			;ccc4   ; ...de las que aqui solo se escriben tres, la columna, la fila...
	inc hl			;ccc5
	ld (hl),b			;ccc6
	inc hl			;ccc7
	ex af,af'			;ccc8   ; ...y el rumbo, que viene en A'. El cuarto se queda como estaba
	ld (hl),a			;ccc9
	scf			;ccca   ; Con carry: ha entrado
	ret			;cccb
persigue_a_cuadros:		; Lo mismo con reja de cuadros y saturacion a mas menos cuatro: corrige siempre en el cuadro 0 de cada 4 y la mitad de las veces en el 2. Al rebotar en 0xB0 rehace el movimiento entero desde la posicion guardada, no solo la columna
	ld (0d3cdh),hl		;cccc   ; La posicion de partida, por si hay que deshacer el paso
	ld a,(0ca8eh)		;cccf
	and 003h		;ccd2
	jr z,L_CCE5		;ccd4
	cp 002h		;ccd6
	...
	jr z,L_CCE5		;ccdf
L_CCE1:
	add hl,bc			;cce1   ; ...y esas tres solo cuestan una suma
	jp L_CD12		;cce2
L_CCE5:
	ld a,h			;cce5   ; La cuarta se corrige, un eje cada vez. La fila del bicho contra la del jugador...
	cp d			;cce6
	ld a,b			;cce7
	jr nc,L_CCF2		;cce8
	add a,004h		;ccea   ; ...y la velocidad vertical sube un paso, saturando en +4
	cp 008h		;ccec
	adc a,000h		;ccee
	jr L_CCF8		;ccf0
L_CCF2:
	add a,004h		;ccf2   ; O baja uno, saturando en -4
	cp 001h		;ccf4
	adc a,0ffh		;ccf6
L_CCF8:
	sub 004h		;ccf8   ; El sesgo de cuatro, quitado: la velocidad vuelve a su rango de -4 a +4
	ld b,a			;ccfa
	ld a,l			;ccfb   ; Y otra vez con la columna
	cp e			;ccfc
	ld a,c			;ccfd
	jr nc,L_CD08		;ccfe
	add a,004h		;cd00
...
L_CD0E:
	...
	add hl,bc			;cd11
L_CD12:
	ld a,l			;cd12   ; El recorte, solo en la columna, con el 0xB0 del area de juego
	cp 0b0h		;cd13
	jr c,L_CD1F		;cd15
	ld a,c			;cd17   ; Rebote: se niega la velocidad horizontal, se repone la posicion de partida y se da el paso otra vez, ya hacia dentro. Al bicho le cuesta el cuadro entero darse la vuelta
	neg		;cd18
	ld c,a			;cd1a
	ld hl,(0d3cdh)		;cd1b
	add hl,bc			;cd1e
...
solapa_eje:		; Solapamiento en UN eje: BC corrige las dos coordenadas (H y L), D y E son los dos tamanos, y vuelve con carry si NO se tocan. Se llama dos veces, una por eje
	add hl,bc			;cd7a   ; BC corrige las dos coordenadas antes de comparar: lo que se mide es la esquina de la caja, no el punto que se pinta
	ld a,l			;cd7b
	cp h			;cd7c   ; Al que va delante se le suma SU tamano: si L queda por debajo de H, se mira si L+E alcanza a H...
	jr nc,L_CD82		;cd7d
	add a,e			;cd7f
	cp h			;cd80
	ret			;cd81
L_CD82:
	ld a,h			;cd82   ; ...y si no, si H+D alcanza a L. Vuelve con acarreo cuando NO se tocan
	add a,d			;cd83
	cp l			;cd84
	ret			;cd85
...
L_CE51:
	...
	pop bc			;ce55
	push bc			;ce56
	ld a,b			;ce57   ; con B = 1 la ficha que se borra es la ultima: no hay nada que subir
	cp 001h		;ce58
	jr z,L_CE4B		;ce5a
	push ix		;ce5c
	pop de			;ce5e
	...
	inc de			;ce62
	inc de			;ce63
	ld hl,0c998h		;ce64   ; 0xC998 es el final de la tabla: lo que va de aqui alli es lo que hay que subir cuatro bytes
	and a			;ce67
	sbc hl,de		;ce68
	ld b,h			;ce6a
	ld c,l			;ce6b
...
mueve_perseguidor:		; Con el estado de 0xD3C5 en 1, apunta la posicion de 0xD3C7 hacia la nave con rumbo_hacia y la mueve de dos en dos
	ld a,(0d3c5h)		;cf36   ; el perseguidor solo se mueve cuando 0xD3C5 dice que ya ha terminado de aparecer
	cp 005h		;cf39
	jr nc,L_CF85		;cf3b
	cp 001h		;cf3d
	ret nz			;cf3f
	ld hl,(0d3c7h)		;cf40   ; se apunta al centro de la nave, ocho pixeles de margen en los dos ejes
	ld bc,00808h		;cf43
	add hl,bc			;cf46
	ld b,h			;cf47
	ld c,l			;cf48
	ld de,(0c184h)		;cf49
	call rumbo_hacia		;cf4d   ; el rumbo se calcula y luego se recorta a las direcciones que el bicho sabe seguir
	call rumbo_a_mascara2		;cf50
	ld hl,(0d3c7h)		;cf53
	ld bc,00202h		;cf56   ; dos pixeles por cuadro en cada eje
	call aplica_rumbo		;cf59
	ld a,l			;cf5c
	cp 0a0h		;cf5d   ; por debajo de la columna 0xA0 sigue; de ahi en adelante se queda donde estaba
	jr c,L_CF65		;cf5f
	ld a,(0d3c7h)		;cf61
	ld l,a			;cf64
L_CF65:
	ld (0d3c7h),hl		;cf65   ; y ademas dispara: uno de cada 64 cuadros, mas o menos
	call azar		;cf68
	and 03fh		;cf6b
	jr nz,L_CF7D		;cf6d
	ld bc,00c0ch		;cf6f   ; el disparo sale doce pixeles por delante, en los dos ejes
	add hl,bc			;cf72
	ld b,h			;cf73
	ld c,l			;cf74
	xor a			;cf75
	...
	ld hl,(0d3c7h)		;cf7a
L_CF7D:
	ld a,019h		;cf7d   ; el sprite 0x19 es el cuerpo del perseguidor
...
choca_con_nave3:		; La tercera caja de contacto con la nave, 5x3 contra 0x16x0A, y la unica que ademas se asegura antes de que la nave siga viva (0xC188 por debajo de 4)
	ld a,(0c188h)		;d0bd   ; la nave explotando no choca con nada
	cp 004h		;d0c0
	ccf			;d0c2
	ret c			;d0c3
	push hl			;d0c4
	ld a,(0c185h)		;d0c5
	ld l,a			;d0c8
	ld bc,00503h		;d0c9   ; la caja de esta comprobacion es de 5 por 3 contra 0x16 por 0x0A: la mas ancha de las tres
	ld de,0160ah		;d0cc
	call solapa_eje		;d0cf   ; primero el eje de las filas y luego el de las columnas
	pop hl			;d0d2
	ret c			;d0d3
	ld h,l			;d0d4
	ld a,(0c184h)		;d0d5
...
recorta_fila:		; La otra mitad del recorte, la de la fila
	...
	ld h,a			;d142
	ret			;d143
borde_pantalla:		; Poda el rumbo en los bordes laterales: con la columna por debajo de 3 quita el bit 3 -la izquierda- y con 174 o mas el bit 2 -la derecha-. El rumbo entra y sale por el A principal, igual que en poda_rumbo_nave; A' solo lo guarda mientras el principal mira la columna
	ex af,af'			;d144   ; El rumbo se aparca en A' mientras el principal mira la columna, igual que en poda_rumbo_nave
	ld a,(ix+000h)		;d145   ; La columna guardada del objeto
	add a,010h		;d148   ; Sumarle 0x10 antes de comparar hace que las columnas de 0xF0 en adelante -las que se han salido por la izquierda- caigan tambien del lado izquierdo
	cp 013h		;d14a   ; Columna por debajo de 3
	jr nc,L_D152		;d14c
	ex af,af'			;d14e
	and 0f7h		;d14f   ; Se apaga el bit 3, la izquierda
	ret			;d151
L_D152:
	cp 0beh		;d152   ; Columna 174 o mas
	jr c,L_D15A		;d154
	ex af,af'			;d156
	and 0fbh		;d157   ; Se apaga el bit 2, la derecha
	ret			;d159
L_D15A:
	ex af,af'			;d15a   ; Ni un borde ni el otro: el rumbo vuelve intacto
	ret			;d15b
recorta_x_objeto:		; Deshace el paso lateral del objeto de IX si se ha salido: con L >= 0xB1 repone L desde (ix+000h). Identica byte a byte a la de la fase de a pie
	ld a,l			;d15c   ; El paso lateral ya dado; el area llega hasta 0xB0
	cp 0b1h		;d15d
	ret c			;d15f
	ld l,(ix+000h)		;d160   ; Si se ha salido, se repone la columna que habia a la entrada, y el objeto se queda pegado al borde
	ret			;d163
arranca_estela:		; Si queda barra (0xD3C1) y la puerta de (iy+000)|(iy+005) esta a cero, gasta un punto, repinta la barra y lanza la estela de 0xD3BD desde la posicion de la nave, con vida 3 y paso 0x03C0 o 0x003C segun la columna. La llama dispara al terminar
	ld iy,0d3bdh		;d164   ; la estela solo arranca si queda carga (iy+4) y no hay otra en marcha
	ld a,(iy+004h)		;d168
	and a			;d16b
	ret z			;d16c
	ld a,(iy+005h)		;d16d
	or (iy+000h)		;d170
	ret nz			;d173
	dec (iy+004h)		;d174   ; una carga menos, y se repinta el indicador
	call pinta_energia		;d177
	ld (iy+000h),003h		;d17a   ; tres cuadros de estela
	ld hl,(0c184h)		;d17e
	ld a,h			;d181
	sub 018h		;d182   ; la estela sale 0x18 filas por encima de la nave
	ld (iy+002h),a		;d184
	sub 008h		;d187
	cp 034h		;d189   ; con un suelo de 0x34: mas arriba no se dibuja
	jr c,L_D18F		;d18b
	ld a,034h		;d18d
L_D18F:
	rrca			;d18f   ; la fila entre dos y la columna entre cuatro: la estela va en la rejilla gruesa
	and 07fh		;d190
	ld (iy+003h),a		;d192
	ld a,l			;d195
	rrca			;d196
	...
	and 01fh		;d19a
	ld (iy+001h),a		;d19c
	cp e			;d19f   ; si la columna cae justa en la rejilla, la mascara es 0x03C0; si no, 0x003C
	ld hl,003c0h		;d1a0
	jr z,L_D1A8		;d1a3
	ld hl,0003ch		;d1a5
L_D1A8:
...
traza_estela:		; Anima la estela del objeto de 0xD3BD en tres fotogramas segun la vida que le queda, y siempre en vertical sobre la misma columna: con vida 3 pinta (iy+003) puntos subiendo desde la base, con vida 2 pasa DOS veces por el nucleo y saca el trazo entero de 2*(iy+003), y con vida 1 sube primero (iy+003) filas SIN pintar -su bucle de 0xD1D3 no escribe en el buffer- y pinta solo la mitad alta. O sea que el chorro crece, llega entero y se despega por abajo. El 24 y el 25 no son "recta o diagonal": los dos valen una fila, y el 25 solo compensa el `inc hl` del segundo byte de cada punto
	ld iy,0d3bdh		;d1af   ; la estela se pinta durante tres cuadros y se va borrando sola
	ld a,(iy+000h)		;d1b3
	and a			;d1b6
	ret z			;d1b7
	ld l,(iy+001h)		;d1b8
	...
	ld de,00019h		;d1c1   ; 0x19 bytes de una fila del buffer a la siguiente
	ld a,(iy+000h)		;d1c4
	dec (iy+000h)		;d1c7
	cp 002h		;d1ca
	push af			;d1cc
...
pinta_tramo_estela:		; El nucleo de traza_estela: pinta (iy+003) puntos de la estela subiendo por el buffer, dos bytes por punto -(iy+006) y (iy+007) con `or (hl)`- y restando DE=25 entre uno y otro. Como el `inc hl` del segundo byte suma 1 antes de la resta, el paso NETO es de 24, el ancho justo de la fila: cada punto cae en la misma columna una fila mas arriba, y la estela sale vertical. traza_estela entra aqui una vez con vida 3, DOS con vida 2 (la llama en 0xD1DB y ademas cae dentro tras el `cp 003h / ret nc` de 0xD1DF), y con vida 1 una sola vez pero despues de subir (iy+003) filas de 24 sin pintar nada
	ld b,(iy+003h)		;d1e2
L_D1E5:
	ld a,(iy+006h)		;d1e5   ; los dos bytes de la mascara se meten con `or`: la estela se suma a lo que ya hay
	or (hl)			;d1e8
	ld (hl),a			;d1e9
	inc hl			;d1ea
	ld a,(iy+007h)		;d1eb
...
impacto_objeto:		; Si el objeto de IX esta marcado y 0xD3BD lo permite, lo pasa al estado 0xC190, le pone 0x28 en (ix+002), suena 0xEA52, descarta la vuelta con `pop hl` y premia 200 puntos. Si no, comprueba el contacto con la nave -y solo si la nave esta viva, 0xC188 por debajo de 4-
	bit 2,(ix+007h)		;d1f5   ; el bit 2 de (ix+7) es el impacto de hace tres cuadros
	jr z,L_D21F		;d1f9
	ld a,(0d3bdh)		;d1fb
	dec a			;d1fe
	jr nz,L_D21F		;d1ff
	ld hl,0c190h		;d201   ; el objeto se convierte en el premio de 0xC190
	ld (ix+003h),l		;d204
	ld (ix+004h),h		;d207
	ld (ix+002h),028h		;d20a   ; 0x28 cuadros de premio en pantalla
	xor a			;d20e
	ld de,0ea52h		;d20f
	call arranca_guion_libre		;d212
	pop hl			;d215
	ld hl,0dd83h		;d216   ; y ademas puntua: dos veces la tabla de 0xDD83
	ld b,002h		;d219
	call premia		;d21b
	ret			;d21e
L_D21F:
	ld a,(0c188h)		;d21f   ; el `sla` del historial de contactos corre los bits: el de arriba se pierde y entra un cero
	sla (ix+007h)		;d222
	cp 004h		;d226
	ret nc			;d228
	ld l,(ix+001h)		;d229
	ld h,(ix+000h)		;d22c
	sla h		;d22f   ; la fila por dos: el objeto va en la rejilla gruesa y la nave en la fina
	call choca_con_nave		;d231
	ret c			;d234
	set 0,(ix+007h)		;d235   ; al chocar se apaga el escudo (0xD3C2)
	xor a			;d239
	ld (0d3c2h),a		;d23a
	ret			;d23d
...
borra_celda:		; Escribe el valor 0x11 ocho veces en dos celdas de VRAM separadas 0x40, con el puerto en C para poder usar `out (c),b`
	ld e,002h		;d2e4
L_D2E6:
	di			;d2e6   ; el historial se recorre con el mismo `sla`
	ld c,099h		;d2e7
	out (c),l		;d2e9
	and a			;d2eb
	and a			;d2ec
	...
	ld d,008h		;d2f2
L_D2F4:
	out (c),b		;d2f4   ; y aqui se mira el bit 2, el contacto de hace tres cuadros
	and a			;d2f6
	dec d			;d2f7
	jr nz,L_D2F4		;d2f8
	ei			;d2fa
...
mata_nave:		; La unica puerta de la muerte: apaga las dos marcas del HUD y mete un 4 en 0xC188, que es lo que arranca la explosion. Diez sitios del listado acaban aqui
	ld hl,02778h		;d303   ; matar la nave es poner 4 en 0xC188, que es el estado de explosion
...
pinta_banda_ruido:		; La brocha de tile_46, y solo suya: pinta B columnas de un byte por 8 filas del buffer, y para CADA columna echa a suertes (`call azar / and 001h`) entre los dos patrones de 8 bytes de 0xD3AD y 0xD3B5, que son los dieciseis primeros de los 34 que figuraban en 0xD3AD como tabla sin dueno. Cada byte lo borra primero con el patron rotado y complementado (`ld a,(de) / rrca / cpl / and (hl)`) y lo pinta despues con `or (hl)`; entre fila y fila suma 24, el ancho del buffer, y entre columna y columna un `inc hl`
	push bc			;d383   ; el dibujo de la banda se sortea entre dos, el de 0xD3AD y el de 0xD3B5, y se pinta en ocho filas
	call azar		;d384
	and 001h		;d387
	ld de,0d3adh		;d389
	jr z,L_D391		;d38c
...
L_D391:
	...
	push hl			;d393
L_D394:
	ld a,(de)			;d394   ; el byte de la banda se aplica en dos pasos: primero se BORRA con la mascara (rotada y complementada) y luego se pinta con `or`
	rrca			;d395
	cpl			;d396
	and (hl)			;d397   ; el `and (hl)` borra y el `or (hl)` de dos lineas mas abajo pinta
	ld (hl),a			;d398
	ld a,(de)			;d399
	or (hl)			;d39a
	ld (hl),a			;d39b
	...
	add a,018h		;d39d
	ld l,a			;d39f
	ld a,h			;d3a0   ; 0x18 bytes de una fila del buffer a la siguiente, con el acarreo al byte alto
	adc a,000h		;d3a1
	ld h,a			;d3a3
	inc de			;d3a4
	djnz L_D394		;d3a5
...
L_D486:
	ld b,008h		;d486
L_D488:
	out (098h),a		;d488   ; El byte va a pelo por el puerto de datos, sin volver a dar la direccion: el VDP la sube solo
	and a			;d48a   ; El `and a` no comprueba nada -el `dec b` de detras vuelve a poner las banderas-: esta para darle tiempo al VDP entre byte y byte
	dec b			;d48b
	jr nz,L_D488		;d48c
	ei			;d48e
	ld de,00040h		;d48f   ; La celda de al lado esta 0x40 mas alla, que en la geometria por columnas de este juego es la columna siguiente
	add hl,de			;d492
	call vram_pon_dir		;d493   ; Y ahi si hay que volver a dar la direccion, que la de antes se quedo ocho bytes mas alla
	dec c			;d496
	jr nz,L_D486		;d497
	ei			;d499
	ret			;d49a
rotula_glifo:		; Estampa el glifo 0x5F00 + A*8 en el buffer a doble altura y con tramado en damero -borra con rlca/cpl/and y pinta con and 0x55/0xAA-, recortando al llegar a 0x4F00
	ld e,a			;d49b
	ld a,h			;d49c   ; El buffer acaba en 0x4EFF: si el rotulo ha caido en 0x4F00 o mas alla, no se pinta
	cp 04fh		;d49d
	ld a,e			;d49f
	ret nc			;d4a0
	cp 020h		;d4a1   ; Este `cp 020h` -mirar si el caracter es un espacio- no lo lee nadie: lo unico que mira banderas detras es el `dec a` del bucle, y las pone el mismo
	push hl			;d4a3
	ld h,000h		;d4a4   ; El glifo son ocho bytes en 0x5F00 + caracter*8
	ld l,a			;d4a6
	add hl,hl			;d4a7
	add hl,hl			;d4a8
	add hl,hl			;d4a9
	...
	push hl			;d4b0
	push de			;d4b1
	ld bc,00018h		;d4b2   ; 0x18 es el ancho del buffer: sumarlo es bajar una fila
	ld a,008h		;d4b5   ; Ocho filas de glifo...
L_D4B7:
	ex af,af'			;d4b7   ; ...y cada una se estampa en DOS filas del buffer: el rotulo sale a doble altura
	ld a,(de)			;d4b8
	rlca			;d4b9   ; Esta pasada BORRA, y borra el glifo corrido un pixel a la izquierda: el `rlca` lo corre y el `cpl` lo pone del reves para el `and`
	cpl			;d4ba
	and (hl)			;d4bb
	ld (hl),a			;d4bc
	add hl,bc			;d4bd
	...
	ld (hl),a			;d4c2
	add hl,bc			;d4c3
	inc de			;d4c4   ; Byte siguiente del glifo
	ex af,af'			;d4c5
	dec a			;d4c6
	jp nz,L_D4B7		;d4c7
	pop de			;d4ca
	pop hl			;d4cb
	push hl			;d4cc   ; La segunda pasada arranca una fila mas abajo que la de borrar, asi que lo borrado asoma una fila por encima del trazo
	add hl,bc			;d4cd
	ld a,008h		;d4ce
L_D4D0:
	ex af,af'			;d4d0
	ld a,(de)			;d4d1
	and 055h		;d4d2   ; Y ahora se pinta con `or`, pero tramado: en una fila los pixeles impares...
	or (hl)			;d4d4
	ld (hl),a			;d4d5
	add hl,bc			;d4d6
	ld a,(de)			;d4d7
	and 0aah		;d4d8   ; ...y en la de al lado los pares, o sea un damero que deja ver el fondo entre trazo y trazo
	or (hl)			;d4da
	ld (hl),a			;d4db
	add hl,bc			;d4dc
	inc de			;d4dd   ; Byte siguiente, igual que en la pasada de borrar
	ex af,af'			;d4de
	dec a			;d4df
	jp nz,L_D4D0		;d4e0
	pop hl			;d4e3
...
rotula_cadena:		; Recorre la cadena de (IX) hasta el 0 estampando cada caracter con 0xD49B y avanzando el destino HL
	ld a,(ix+000h)		;d4e5   ; Un caracter de la cadena de IX...
	inc ix		;d4e8
	and a			;d4ea
	ret z			;d4eb   ; ...y el cero la cierra
	call rotula_glifo		;d4ec
	inc hl			;d4ef   ; Un byte del buffer por caracter: los glifos van pegados
...
pinta_menu:		; Estampa las cinco lineas del menu -STARDUST, JOYSTICK, TECLADO, REDEFINIR TECLAS, JUGAR, en 0xDCC6- sobre el buffer en 0x40C8, 0x43C2, 0x46C2, 0x49C2 y 0x4CCA, y con 0xDCC3 puesto anade el selector "<>>>>>>>" de 0xDCF7 en la opcion elegida
	ld ix,0dcc6h		;d4f3   ; Las cinco cadenas van SEGUIDAS desde 0xDCC6, y cada una rotulada deja IX apuntando a la siguiente: por eso el menu entero se pinta con un solo IX y cinco destinos
	ld hl,040c8h		;d4f7
	...
	ld a,(0dcc3h)		;d515   ; Con 0xDCC3 apagado el selector va en TECLADO y con el encendido en JOYSTICK, cada uno con su cadena de flechas
	and a			;d518
	jr z,L_D525		;d519
	ld ix,0dcf7h		;d51b
	ld hl,043c2h		;d51f
...
scroll_records:		; Un cuadro de la pantalla de records: baja el puntero de 0xDCC4 una fila del buffer (24 bytes) y vuelve a rotular el titulo "STARDUST" de 0xDCC6 y las ocho fichas de 0xDD08, cada una 384 bytes por debajo de la anterior, que a 24 por fila son las 16 filas justas que ocupa un glifo a doble altura. Cuando el puntero se sale por arriba del buffer (`ld a,h / cp 040h`) se va a 0xD591, que desapila la vuelta, arranca la musica, espera a que acabe y entra en la demo sembrando el azar con 0xA710 y parcheando 0xC090 con lee_mando_demo. El llamador es el bucle de 0xD566, que arranca poniendo 0xDCC4 en 0x4F08. Gemela de la scroll_records de la fase de a pie: 35 de sus 49 bytes son identicos
	ld hl,(0dcc4h)		;d52f   ; El puntero donde se rotula, que el bucle de 0xD560 arranca en 0x4F08, o sea fila 160 -justo debajo del buffer- y columna 8
	ld de,00018h		;d532   ; Restarle el ancho del buffer es subir el rotulo una fila: eso es todo el scroll
	and a			;d535
	sbc hl,de		;d536
	ld a,h			;d538
	cp 040h		;d539   ; Y por debajo de 0x4000 el rotulo entero se ha ido por arriba
	jp c,L_D591		;d53b
	ld (0dcc4h),hl		;d53e
	ld ix,0dcc6h		;d541   ; El titulo, la primera de las cadenas encadenadas de 0xDCC6
	call rotula_cadena		;d545
	ld de,00235h		;d548   ; Del titulo a la primera ficha: sus 8 caracteres mas 0x235 son 573 bytes, que a 24 por fila son 24 filas mas abajo y la columna 5
	add hl,de			;d54b
	ld ix,0dd08h		;d54c   ; Las ocho fichas de record, que van seguidas: ocho de nombre, seis de puntos y el cero
	ld de,00172h		;d550   ; De una ficha a la de abajo: sus 14 caracteres mas 0x172 son 384 justos, o sea 16 filas, que es lo que mide un renglon a doble altura
	ld b,008h		;d553
L_D555:
	push bc			;d555
	push de			;d556
	call rotula_cadena		;d557   ; Nombre y puntos se rotulan de una vez: son la misma cadena
	pop de			;d55a
	add hl,de			;d55b   ; Y a la ficha siguiente
	pop bc			;d55c
	djnz L_D555		;d55d
	ret			;d55f
L_D560:
	...
	ld (0dcc4h),hl		;d563
L_D566:
	call borra_buffer		;d566   ; El cuadro de la pantalla de records, en cuatro pasos: borrar el buffer, mover las estrellas, subir el rotulo un escalon y volcar
	call mueve_estrellas		;d569
	call scroll_records		;d56c
	call vuelca_pantalla		;d56f
	call hay_tecla		;d572   ; Con cualquier tecla se corta y se vuelve al menu
...
L_D57F:
	call hay_tecla		;d57f
	ret nz			;d582
	ld bc,003e8h		;d583   ; Mil vueltas de espera entre cata y cata del teclado
	call espera_bc		;d586
	ld hl,(0eda3h)		;d589   ; Y el final de la musica se nota en que el canal 1 se queda sin puntero de ejecucion
	ld a,h			;d58c
	or l			;d58d
	jr nz,L_D57F		;d58e
	ret			;d590
L_D591:
	pop hl			;d591   ; El `pop hl` se come la vuelta al bucle de 0xD566: aqui no se vuelve a scroll_records, se sale de la pantalla de records
	nop			;d592   ; Tres `nop` seguidos, con pinta de que algo se quito a mano
	nop			;d593
	nop			;d594
	call arranca_musica		;d595   ; La musica de la demo, y aqui se espera a que acabe
	call espera_fin_musica		;d598
	ld hl,0a710h		;d59b   ; La semilla del azar se planta en 0xA710 SIEMPRE, asi que la demo sale igual todas las veces
	ld (0ca8fh),hl		;d59e
	ld hl,0c1afh		;d5a1   ; Y el mando pasa a leerse de la demo: 0xC1AF en el operando del `call` de 0xC090, que el arranque de partida vuelve a poner en lee_mando (0xBE1B)
	ld (0c090h),hl		;d5a4
	jp hud_reset		;d5a7
L_D5AA:
...
L_D9CA:
	ld a,(0ca8eh)		;d9ca   ; el tile parpadea uno de cada cuatro cuadros, y el parpadeo es un `xor 1` sobre su byte
	and 003h		;d9cd
	ret nz			;d9cf
	ld l,(ix+005h)		;d9d0
	ld h,(ix+006h)		;d9d3
...
choca_con_nave4:		; La cuarta caja de contacto con la nave, y la unica alargada: 4x2 contra 0x38 x 0x0C en el primer eje y 7 contra 0x12 en el segundo
	ld a,(0c188h)		;d9db   ; la nave explotando ya no choca
	cp 004h		;d9de
	ret nc			;d9e0
	ld h,(ix+000h)		;d9e1
	sla h		;d9e4   ; la fila del objeto por dos: va en la rejilla gruesa
	ld a,(0c185h)		;d9e6
	ld l,a			;d9e9
	ld de,0380ch		;d9ea   ; las cajas: 0x38 por 0x0C de alto y 4 por 2 de ancho
	ld bc,00402h		;d9ed
	...
	ret c			;da02
	jp mata_nave		;da03   ; y si se tocan, la nave muere
descomprime_token:		; Expande un token del mapa comprimido: si el bit 7 esta a cero es un literal, que escribe en (ix+000) y avanza IX; si esta a uno, sus siete bits bajos son el numero de frase del diccionario de 0xDE18, adonde llega saltando de frase en frase por el tamano que cada una lleva en su primer byte, y la expande LLAMANDOSE A SI MISMA una vez por cada token que contenga, de modo que una frase puede contener otras. El 0xFF cierra el flujo y se come la vuelta con el `pop hl` de 0xDA31
	ld a,(hl)			;da06   ; el descompresor: 0xFF acaba, los valores positivos van tal cual y los negativos son un token
	cp 0ffh		;da07
	jr z,L_DA31		;da09
	jp p,L_DA2A		;da0b
	push hl			;da0e
	ld hl,0de18h		;da0f   ; la tabla de tokens de 0xDE18 se recorre saltando lo que diga cada cabecera
	ld d,000h		;da12
	and 07fh		;da14
	jr z,L_DA1D		;da16
	ld b,a			;da18
...
L_DA1D:
	...
	inc hl			;da1f
L_DA20:
	push bc			;da20   ; y cada token se expande llamando otra vez al mismo descompresor
	...
	ret			;da29
L_DA2A:
	ld (ix+000h),a		;da2a   ; los positivos se copian al destino y ya
	inc ix		;da2d
	inc hl			;da2f
	ret			;da30
L_DA31:
...
tile_5D:		; La rutina de gobierno que el despachador de 0xC170 le da al tile 0x5D (`cp 05dh / ld de,0da3dh`), el que cierra la zona 7. Al decimo cuadro de vida -(ix+000), que recorre_tiles_especiales sube uno por cuadro, contra el `cp 00ah / ret c` de 0xDA40- CONGELA EL SCROLL poniendo 0xCB04 a cero, que es la puerta que mira repinta_fondo en 0xC42A antes de bajar una fila del mapa, levanta la bandera 0xDAC5 -que corta la aparicion de enemigos sueltos en 0xD420 y la creacion de tandas nuevas en 0xD82C- y se cambia a si misma la rutina de gobierno por la de 0xDA72
	ld a,(ix+000h)		;da3d   ; el tile 0x5D no hace nada hasta llegar a la fila 10
	cp 00ah		;da40
	ret c			;da42
	xor a			;da43   ; y entonces se cambia a si mismo la rutina: pasa a esperar
	ld (0cb04h),a		;da44
	inc a			;da47
	ld (0dac5h),a		;da48
...
zona_despejada:		; Vuelve con Z solo si no queda nada en pantalla: 0xCA91 en 0x80 y los cinco contadores de objetos -0xC98F, 0xC97A, 0xC939, 0xC998 y 0xD3C5- todos a cero, juntados con OR
	ld a,(0ca91h)		;da55   ; la zona esta despejada cuando no queda ninguna de las cuatro listas de bichos ni el perseguidor
	cp 080h		;da58
	ret nz			;da5a
	ld a,(0c98fh)		;da5b
	ld e,a			;da5e
	ld a,(0c97ah)		;da5f   ; las dos primeras listas de bichos
	or e			;da62
	ret nz			;da63
	ld a,(0c939h)		;da64
	ld e,a			;da67
	ld a,(0c998h)		;da68   ; y las otras dos
	or e			;da6b
	ret nz			;da6c
	ld a,(0d3c5h)		;da6d
	or e			;da70
...
tile_5D_espera:		; El 0x5D esperando: cada cuadro repone (ix+000) a cero para no caducar nunca en el `cp 060h` de recorre_tiles_especiales, y no hace nada hasta que se dan tres condiciones a la vez -que zona_despejada diga que no queda nada, que la nave este en la columna 0x58 (la misma en la que la deja el arranque de zona con `ld hl,0a058h`) y que 0xC9A3 valga exactamente 8-. Cuando se cumplen, parchea el operando del `jp` de va_a_nave_estado (0xC064) para que el bucle principal llame a arrastra_nave en vez de a la nave, parchea el de pinta_escudo (0xD0DD) con el `ret` de 0xD959 para apagar el indicador -los dos mismos trampolines que hud_reset repone en 0xBE9F y 0xBEA8-, devuelve el scroll poniendo 0xCB04 = 8, siembra 0xC9A3 con 0x1F y se da de baja
	call zona_despejada		;da72   ; con la zona despejada, el tile se apaga
	ld (ix+000h),000h		;da75
	ret nz			;da79
	ld a,(0c184h)		;da7a   ; y ademas la nave tiene que estar en la columna 0x58 y el contador en 8
	cp 058h		;da7d
	ret nz			;da7f
	ld a,(0c9a3h)		;da80
	cp 008h		;da83
	ret nz			;da85
	ld hl,arrastra_nave		;da86   ; aqui se cambian dos rutinas del bucle de partida: la nave pasa a ser arrastrada y el escudo deja de pintarse
	ld (0c064h),hl		;da89
	ld hl,ld959h		;da8c
	ld (0d0ddh),hl		;da8f
	ld (0cb04h),a		;da92
...
arrastra_nave:		; Lo que sustituye al gobierno de la nave en el cierre de la zona 7: pinta el sprite 8 en la posicion de la nave (0xC184/85) y el sprite 0x18 en esa misma columna a la altura que marca 0xC9A3 -reutilizado como coordenada desde que tile_5D_espera lo puso a 0x1F-, y baja esa altura de dos en dos cuadro a cuadro hasta el tope de 0xC8. En cuanto alcanza a la nave (`cp h` contra la Y de 0xC185) le impone su propia fila, o sea que la arrastra hacia abajo y la saca de la banda de juego, que acaba en 0xB0. Los dos sprites NO son la misma clase de cosa, aunque caigan en el mismo tramo: el 8 es la nave del jugador quieta y mirando hacia arriba -gira_rumbo remata con `or 008h`, o sea que los reposos de los ocho rumbos son 0x08 a 0x0F, y el rumbo 0 es arriba-, y por eso el relevo es continuo, con tile_5D_espera exigiendo justamente 0xC9A3 == 8 antes de disparar la escena; el 0x18, en cambio, no es ningun fotograma de vuelo -la aritmetica de gira_rumbo no lo alcanza nunca- sino CUATRO BARRAS HORIZONTALES blancas con trama de ajedrez entre ellas, un haz rayado del ancho de la nave, y el unico sitio de todo el bloque que lo pinta es esta rutina. O sea que lo que baja es el HAZ y lo que se arrastra es la NAVE
	ld hl,(0c184h)		;da9d   ; el arrastre pinta la nave y, encima, el sprite 0x18
	push hl			;daa0
	ld a,008h		;daa1
	...
	pop af			;dab2
	pop hl			;dab3
	cp 0c8h		;dab4   ; sube de dos en dos filas hasta 0xC8
	ret nc			;dab6
	inc a			;dab7
	inc a			;dab8
	cp h			;dab9
	...
	ld h,a			;dabc
L_DABD:
	ld (0c9a3h),a		;dabd   ; y la fila de la nave se iguala a la del arrastre: la nave va detras
	ld a,h			;dac0
	ld (0c185h),a		;dac1
	ret			;dac4
...
arranca_musica:		; Instala la musica de la partida de golpe: canal 0 <- 0xEB52, canal 1 <- 0xEC4A, canal 2 <- 0xECCB. Medido en el emulador, es exactamente lo que se ve arrancar (ver el bloque de la partitura)
	ld a,080h		;e16f   ; la musica del menu son tres guiones, uno por canal
	ld de,0eb52h		;e171
...
tic_sonido:		; El motor del sonido, que la interrupcion llama cincuenta veces por segundo: recorre los tres canales desde 0xED75, gasta la duracion de (ix+004/005) y, al agotarse, apaga el mezclador y va a por el siguiente comando del guion
	push af			;e1f5   ; Los tres canales, uno detras de otro
	ld b,003h		;e1f6
	xor a			;e1f8
	ld ix,0ed75h		;e1f9   ; IX al primer bloque de canal, DE a los registros de periodo del PSG (0xEE0D es el R0) y HL a los de volumen (0xEE15 es el R8): el bloque sombra es la imagen de los once registros
	ld de,0ee0dh		;e1fd
	ld hl,0ee15h		;e200
L_E203:
	push af			;e203
	...
	ld b,(ix+003h)		;e21a
	ld a,b			;e21d   ; Sin puntero no hay guion: canal muerto
	or c			;e21e
	jp z,L_E327		;e21f   ; ...y si es cero, este canal no tiene nada que interpretar
L_E222:
	ld a,(bc)			;e222   ; El byte al que apunta el puntero de ejecucion, que vive en BC
...
lee_nota:		; Un byte por debajo de 0x80 es una nota: le suma la transposicion de 0xEE21+canal, busca el periodo en la tabla de 0xE6E3 y lo deja en (ix+00A/00B)
	push af			;e231   ; La nota se apila porque entrada_transporte machaca A con el numero de canal: vuelve en el `pop af` de 0xE235, justo para sumarle la transposicion
...
ataca_nota:		; El ataque: abre tono y ruido en el mezclador, monta la envolvente del instrumento y la pone a cero. Es justo lo que se salta el comando 0x84
	...
	ld (ix+02ah),000h		;e24d   ; ...y lo que las dos envolventes llevan sumado al volumen y al periodo arranca a cero
...
arranca_duracion:		; Guarda el puntero del guion en (ix+002/003) y recarga la cuenta atras de la duracion (ix+004/005) desde (ix+006/007)
	...
	ld (ix+005h),h		;e26b
L_E26E:
	ld l,(ix+004h)		;e26e   ; Un cuadro menos de nota, siempre, suene lo que suene
	ld h,(ix+005h)		;e271
	dec hl			;e274
	ld (ix+004h),l		;e275
	ld (ix+005h),h		;e278
	push ix		;e27b   ; IY recorre las fases mientras IX se queda en la cabecera del bloque
	pop iy		;e27d
	ld d,002h		;e27f   ; La envolvente de volumen tiene DOS fases
	ld c,000h		;e281
L_E283:
	ld a,(iy+00ch)		;e283   ; Si a la fase le faltan cuadros para el paso siguiente, se descuenta uno y se acabo el cuadro
	or a			;e286
	jr z,L_E290		;e287
	dec a			;e289
	ld (iy+00ch),a		;e28a
	...
	jr L_E2B1		;e28e
L_E290:
	ld a,(iy+00eh)		;e290   ; Y si ya toca paso, se mira cuantos le quedan a la fase
	or a			;e293
	jr z,L_E2AC		;e294
	dec a			;e296
	ld (iy+00eh),a		;e297
	ld a,(ix+02ah)		;e29a   ; El paso: al volumen acumulado se le suma lo que diga la fase
	add a,(iy+01bh)		;e29d
	ld (ix+02ah),a		;e2a0
	ld a,(iy+020h)		;e2a3   ; Y se recarga la espera desde la plantilla
	ld (iy+00ch),a		;e2a6
	inc c			;e2a9
	jr L_E2B1		;e2aa
L_E2AC:
	inc iy		;e2ac   ; Fase agotada: a la siguiente
	dec d			;e2ae
	jr nz,L_E283		;e2af
L_E2B1:
	ld a,c			;e2b1   ; Si ninguna de las dos fases hizo nada, la envolvente esta terminada...
	or a			;e2b2
	jr nz,L_E2BC		;e2b3
	bit 0,(ix+02dh)		;e2b5   ; ...y el bit 0 del +2D decide si vuelve a empezar o se queda ahi
	call nz,carga_envolvente_1		;e2b9
L_E2BC:
	push ix		;e2bc
	pop iy		;e2be
	ld d,003h		;e2c0   ; La de periodo va igual, pero con TRES fases
	ld c,000h		;e2c2
L_E2C4:
	ld a,(iy+010h)		;e2c4   ; La espera de la fase
	or a			;e2c7
	jr z,L_E2D1		;e2c8
	dec a			;e2ca
	ld (iy+010h),a		;e2cb
	...
	jr L_E31C		;e2cf
L_E2D1:
	ld a,(iy+013h)		;e2d1   ; Los pasos que le quedan
	or a			;e2d4
	jr z,L_E317		;e2d5
	dec a			;e2d7
	ld (iy+013h),a		;e2d8
	ld a,(iy+01dh)		;e2db   ; El paso es con signo: si el delta es negativo...
	or a			;e2de
	jp p,L_E2FB		;e2df
	ld a,(iy+01dh)		;e2e2   ; ...se le da la vuelta y se RESTA con acarreo de los dos bytes
	cpl			;e2e5
	inc a			;e2e6
	ld e,a			;e2e7
	ld a,(ix+02bh)		;e2e8
	...
	ld a,(ix+02ch)		;e2ef
	sbc a,000h		;e2f2
	and 00fh		;e2f4   ; El periodo del PSG son doce bits: el `and 00fh` recorta el byte alto
	ld (ix+02ch),a		;e2f6
	jr L_E30E		;e2f9
L_E2FB:
	ld a,(ix+02bh)		;e2fb   ; Y con delta positivo, la suma de siempre
	add a,(iy+01dh)		;e2fe
	ld (ix+02bh),a		;e301
	ld a,(ix+02ch)		;e304
	adc a,000h		;e307
	...
	ld (ix+02ch),a		;e30b
L_E30E:
	ld a,(iy+022h)		;e30e   ; La espera, recargada desde la plantilla
	ld (iy+010h),a		;e311
	inc c			;e314
	jr L_E31C		;e315
L_E317:
	inc iy		;e317   ; Fase agotada: a la siguiente de las tres
	dec d			;e319
	jr nz,L_E2C4		;e31a
L_E31C:
	ld a,c			;e31c
...
L_E327:
	...
	pop de			;e349
	pop af			;e34a
	inc a			;e34b   ; El numero de canal sube -es lo que mira el mezclador- y HL pasa al registro de volumen de al lado
	inc hl			;e34c
	dec b			;e34d   ; Los tres canales
	jp nz,L_E203		;e34e
	ld iy,0edffh		;e351   ; Y detras empieza otro recorrido de tres, este sobre 0xEDFF
	ld d,002h		;e355   ; Dos fases, y el mismo bucle que la de volumen
	ld c,000h		;e357
L_E359:
	ld a,(iy+000h)		;e359   ; La espera de la fase...
	or a			;e35c
	jr z,L_E366		;e35d
	dec a			;e35f
	ld (iy+000h),a		;e360
	...
	jr L_E387		;e364
L_E366:
	ld a,(iy+002h)		;e366   ; ...y los pasos que le quedan
	or a			;e369
	jr z,L_E382		;e36a
	dec a			;e36c
	ld (iy+002h),a		;e36d
	ld a,(0ee0bh)		;e370   ; El paso se suma en 0xEE0B, que es lo que la envolvente lleva puesto al periodo de ruido
	add a,(iy+006h)		;e373
	ld (0ee0bh),a		;e376
	ld a,(iy+008h)		;e379
	ld (iy+000h),a		;e37c
	...
	jr L_E387		;e380
L_E382:
	inc iy		;e382   ; Fase agotada: a la otra
	dec d			;e384
	jr nz,L_E359		;e385
L_E387:
	ld a,c			;e387
	or a			;e388
	jr nz,L_E393		;e389
	ld a,(0ee09h)		;e38b   ; Si ninguna hizo nada, el bit 2 de las banderas globales decide si se recarga y vuelve a empezar
	bit 2,a		;e38e
	call nz,refresca_globales_sonido		;e390
L_E393:
	ld a,(0ee0ah)		;e393   ; Lo que llega al chip es el periodo de ruido que puso op_ruido mas lo que lleve la envolvente
	ld e,a			;e396
	ld a,(0ee0bh)		;e397
	add a,e			;e39a
	ld (0ee13h),a		;e39b
	call vuelca_psg		;e39e   ; Y ahora si, los once registros de la sombra al PSG: una sola vez por interrupcion, con los tres canales ya resueltos
	pop af			;e3a1
	ret			;e3a2
carga_envolvente_1:		; Copia dos parejas de la plantilla del instrumento a las variables vivas: (ix+020) a (ix+00C) y (ix+016) a (ix+00E)
	...
	ld d,002h		;e3a5
L_E3A7:
	ld a,(ix+020h)		;e3a7   ; Dos parejas: la espera desde el +20 y los pasos desde el +16
	ld (ix+00ch),a		;e3aa
	ld a,(ix+016h)		;e3ad
	ld (ix+00eh),a		;e3b0
	inc ix		;e3b3   ; El `inc ix` pasa a la fase siguiente, que van byte a byte
	dec d			;e3b5
	jr nz,L_E3A7		;e3b6
	pop ix		;e3b8
	ret			;e3ba
carga_envolvente_2:		; Lo mismo con tres: (ix+022) a (ix+010) y (ix+018) a (ix+013)
	...
	push ix		;e3bd
L_E3BF:
	ld a,(ix+022h)		;e3bf   ; Y aqui tres parejas, desde el +22 y el +18
	ld (ix+010h),a		;e3c2
	ld a,(ix+018h)		;e3c5
	ld (ix+013h),a		;e3c8
	inc ix		;e3cb
...
refresca_globales_sonido:		; Copia (iy+008) a (iy+000) y (iy+004) a (iy+002) dos veces desde 0xEDFF: las variables globales del sonido que el fin de voz limpia
	...
	ld iy,0edffh		;e3d7
L_E3DB:
	ld a,(iy+008h)		;e3db   ; Las globales del ruido se rearman igual, con sus dos fases
	ld (iy+000h),a		;e3de
	ld a,(iy+004h)		;e3e1
	ld (iy+002h),a		;e3e4
	inc iy		;e3e7
...
L_E41E:
	...
	ld a,(0ee19h)		;e42a   ; El canal que acaba de callarse...
	ld hl,0ee0ch		;e42d   ; ...se compara con el que anoto op_efecto en 0xEE0C (0xE523): 0xEE0C no cuenta voces vivas, guarda el DUENO del barrido de ruido, y si no es este canal el barrido es de otro y no se toca nada
	xor (hl)			;e430
	jp nz,L_E327		;e431
	ld hl,0edffh		;e434   ; Y si el que acaba es el dueno, a cero las variables comunes del sonido...
	ld de,0ee00h		;e437
	ld bc,0000ah		;e43a
	ld (hl),a			;e43d
...
op_ruido:		; 0x88 n: 0xEE0A = argumento and 0x1F, los cinco bits del registro de ruido del PSG. El bit 7 del argumento decide por donde sigue el interprete
	...
	ld (0ee0ah),a		;e464
	call refresca_globales_sonido		;e467   ; Cambiar el periodo de ruido rearma de paso la envolvente global: las dos fases vuelven a su plantilla
...
op_bucle:		; 0x82: vuelve al principio del guion recargando (ix+002/003) con (ix+000/001). NO es un terminador: solo acaba si va por el canal 1 y ademas 0xEE1A esta a cero
	ld a,(0ee19h)		;e477   ; Solo el canal 1 se pregunta si la musica tiene que acabarse...
	dec a			;e47a
	jr nz,L_E483		;e47b
	ld a,(0ee1ah)		;e47d   ; ...y acaba de verdad cuando 0xEE1A esta apagado, que es como espera_fin_musica (0xD57B) le pide el final
	and a			;e480
	jr z,op_fin		;e481
L_E483:
	ld c,(ix+000h)		;e483   ; Si no, el puntero de ejecucion vuelve al de inicio y el guion empieza otra vez
	ld b,(ix+001h)		;e486
	ld (ix+002h),c		;e489
	ld (ix+003h),b		;e48c
	jp L_E222		;e48f
...
op_instrumento:		; 0x87 n: copia los QUINCE bytes del instrumento n -que vive en 0xE5E2 + n*15- a (ix+016) y siguientes, y de paso limpia los bits 0 y 1 de las banderas
	...
	ld d,00fh		;e4bc
L_E4BE:
	ld a,(hl)			;e4be   ; Se copian a partir del +16, que es donde vive la plantilla del instrumento
	ld (ix+016h),a		;e4bf
	inc hl			;e4c2
	inc ix		;e4c3
	dec d			;e4c5
...
lee_puntero:		; HL = la palabra que hay en HL + A*2: el acceso a las tablas de punteros del interprete de guiones
	push af			;e5c0
	add a,a			;e5c1   ; El indice por dos, que las entradas de la tabla son palabras
	add a,l			;e5c2
	ld l,a			;e5c3
	jr nc,L_E5C7		;e5c4
	inc h			;e5c6
L_E5C7:
	ld a,(hl)			;e5c7   ; Y la palabra, leida en dos pasos para no gastar mas registros
	inc hl			;e5c8
	ld h,(hl)			;e5c9
	ld l,a			;e5ca
	pop af			;e5cb
...
vuelca_psg:		; El volcado al chip, y el unico `out (0a1h),a` de sonido del bloque -el otro, 0xC1D1, es la lectura del joystick-: escribe los ONCE bytes del bloque sombra de 0xEE0D-0xEE17 en los registros 0 a 10 del PSG, con `ld a,000h` de primer registro y `ld d,00bh` de cuenta, sacando el numero por 0xA0 y el dato por 0xA1. Lo llama tic_sonido como ultima cosa de cada tic (`call L_E5CD` en 0xE39E), justo despues de componer el ruido en 0xEE13 sumando la base de 0xEE0A y el barrido de 0xEE0B. La sombra cierra al byte: 0xEE0D-0xEE12 los tres periodos de tono, 0xEE13 el ruido, 0xEE14 el mezclador y 0xEE15-0xEE17 los tres volumenes; los 21 bytes son los de su gemela 0xC8E2 salvo el operando del `ld hl`
	...
	ld d,00bh		;e5d2
L_E5D4:
	push af			;e5d4   ; un byte por vuelta al puerto 0xA0/0xA1: es el volcado al PSG registro a registro
	ld c,(hl)			;e5d5
	out (0a0h),a		;e5d6
	ld a,c			;e5d8
	out (0a1h),a		;e5d9
	pop af			;e5db
	inc a			;e5dc   ; el registro siguiente del PSG
	inc hl			;e5dd
	dec d			;e5de
	jr nz,L_E5D4		;e5df
	ret			;e5e1
...
vram_pon_dir:		; Fija la direccion de ESCRITURA del VDP por el puerto 0x99, con el bit 0x40 puesto
	di			;ee24   ; poner la direccion de VRAM para escribir: el bit 6 en el segundo byte
	push af			;ee25
	in a,(099h)		;ee26   ; el `in a,(0x99)` no lee nada util: es para dejar el VDP en su primer byte de direccion
	ld a,l			;ee28
	out (099h),a		;ee29
	ld a,h			;ee2b
	and 07fh		;ee2c
...
presentacion:		; La presentacion, lo primero que hace el juego (call en 0xBDA6): borra los 6144 B de patrones, pone los de color a 0x71, enciende la pantalla y recorre los 96 pares de 0xF126 llamando a anima_logo, hasta el 0xFF de 0xF1E6; luego caen los creditos
	ld b,032h		;ee34   ; la presentacion: se limpia la pantalla y se enciende
	call retardo		;ee36
	ld hl,00000h		;ee39
	ld bc,01800h		;ee3c
	xor a			;ee3f
	call vram_rellena		;ee40
	ld hl,02000h		;ee43   ; los colores a 0x71, blanco sobre negro
	ld bc,01800h		;ee46
	ld a,071h		;ee49
	...
	ld a,081h		;ee53
	out (099h),a		;ee55
	ld ix,0f120h		;ee57   ; la tabla de 0xF126 lleva los pasos de la animacion del logotipo
	ld hl,0f126h		;ee5b
	ld a,(hl)			;ee5e
	ld (ix+001h),a		;ee5f
L_EE62:
	ld a,(hl)			;ee62   ; un 0xFF cierra la lista y se pasa a los creditos
	cp 0ffh		;ee63
	jr z,creditos		;ee65
	ld (ix+000h),a		;ee67
	inc hl			;ee6a
	...
	inc hl			;ee6f
	push hl			;ee70
	ld b,001h		;ee71   ; una pausa corta entre paso y paso
	call retardo		;ee73
...
L_EECC:
	ld b,002h		;eecc
L_EECE:
	push bc			;eece   ; dos filas de caracter por tercio, 0x100 bytes cada una
	ld bc,00100h		;eecf
	ld a,011h		;eed2
	...
	pop bc			;eedb
	djnz L_EECE		;eedc
	ld de,00100h		;eede   ; y 0x100 mas para caer en la fila siguiente
	add hl,de			;eee1
	dec c			;eee2
	jr nz,L_EECC		;eee3
	ld hl,02100h		;eee5
	ld e,018h		;eee8
L_EEEA:
	ld a,011h		;eeea   ; veinticuatro tiras de ocho bytes: el borde izquierdo del marco
	ld bc,00008h		;eeec
	...
	ld e,018h		;eefc
L_EEFE:
	ld a,011h		;eefe   ; y otras veinticuatro de 0x18: el borde derecho
L_EF00:
	ld bc,00018h		;ef00
...
L_EF2D:
	...
	push hl			;ef48
L_EF49:
	push bc			;ef49   ; la segunda mitad del marco: 24 tiras de ocho bytes
	...
	ld b,018h		;ef5c
L_EF5E:
	push bc			;ef5e   ; y las ultimas 24, de 0x18 bytes
	ld bc,00018h		;ef5f
...
creditos_transicion:		; Despide el cartel deslizandolo hacia arriba, y lo hace moviendo la tabla de NOMBRES (256 bytes) en vez de los patrones (2048): ocho pasos de 0x20 = una fila. Al final borra los dibujos del tercio y reconstruye la tabla con el intercalado de a 8 de la pantalla de carga
	ld hl,04100h		;ef6d   ; la transicion de los creditos: la fila se rellena consigo misma con un `ldir` solapado
	ld de,04101h		;ef70
	ld bc,0001fh		;ef73
	ldir		;ef76
	ld de,04000h		;ef78
	ld hl,01900h		;ef7b   ; se lee la tabla de nombres, se corre 0x20 casillas y se vuelve a escribir: el cartel sube una fila
	ld bc,00100h		;ef7e
	call vram_lee		;ef81
	ld a,008h		;ef84
L_EF86:
	ex af,af'			;ef86   ; ocho veces, o sea ocho filas de subida
	ld de,04000h		;ef87
	ld hl,04020h		;ef8a
	ld bc,00100h		;ef8d
	ldir		;ef90
	ld hl,01900h		;ef92   ; y el resultado se devuelve a la VRAM
	ld de,04000h		;ef95
	ld bc,00100h		;ef98
	call vram_escribe		;ef9b
	ld b,00ah		;ef9e   ; una pausa corta entre fila y fila
	call retardo		;efa0
	ex af,af'			;efa3
	dec a			;efa4
	jr nz,L_EF86		;efa5
	...
	xor a			;efb9
L_EFBA:
	out (c),a		;efba   ; el `add a,8` con el `jr nc` recorre los 32 valores de ocho en ocho, dando la vuelta: es un barrido, no una cuenta
	add a,008h		;efbc
	jr nc,L_EFBA		;efbe
	inc a			;efc0
	cp 008h		;efc1
...
retardo:		; Espera activa: B vueltas de un bucle de 0x1F4
	push bc			;efc6   ; el retardo es un bucle vacio de 0x1F4 vueltas, repetido B veces
	ld bc,001f4h		;efc7
L_EFCA:
	dec bc			;efca   ; la cuenta pelada: ni mira teclado ni espera al VDP
	ld a,b			;efcb
	or c			;efcc
	jr nz,L_EFCA		;efcd
	pop bc			;efcf
...
anima_logo:		; Un fotograma del logo sobre el estado de 0xF120: compara fila y altura con las del fotograma anterior, calcula en (ix+004) cuantas filas hay que borrar y SE PARCHEA los operandos de los dos `call` de 0xF015/0xF018 para dibujar antes de borrar si el logo sube, o al reves si baja
	ld a,(ix+000h)		;efd3   ; la animacion del logotipo compara el paso nuevo con el anterior y elige estirar o borrar
	sub (ix+001h)		;efd6
	...
	jr c,L_EFF0		;efe2
	jr nz,L_EFFD		;efe4
	ld a,(ix+002h)		;efe6   ; con los dos iguales, la comparacion pasa al otro par de bytes
	cp (ix+003h)		;efe9
	ld a,000h		;efec
	jr nc,L_EFFD		;efee
L_EFF0:
	ld c,(ix+000h)		;eff0   ; el orden se le da la vuelta: lo que crecia ahora se borra
	neg		;eff3
	sub (ix+002h)		;eff5
	add a,(ix+003h)		;eff8
	ex de,hl			;effb
	inc a			;effc
L_EFFD:
	ld (ix+004h),a		;effd   ; (ix+004) es cuantas filas hay que mover
	ld (0f016h),hl		;f000   ; y aqui se PARCHEAN las dos llamadas de 0xF015 y 0xF018: la animacion se escribe a si misma que rutina usar
	ld (0f019h),de		;f003
	ld d,c			;f007
	ld e,008h		;f008
	call dir_vram_de_fila_columna		;f00a   ; la fila y la columna 8 dan la direccion de VRAM donde empieza el logotipo
	xor a			;f00d
	ld (0f125h),a		;f00e
	push hl			;f011
	ld de,04000h		;f012
...
logo_estira:		; Estira el logo de 0x47A0 -16 filas de 16 B- sobre el montaje que apunta DE, repartiendolo en (ix+002) filas con un acumulador que arranca en 8: con altura 16 sale copia fila a fila, y con altura 1, dos filas
	ld a,020h		;f02c   ; el estirado: 0x20 menos la posicion, y dieciseis filas
	ld b,010h		;f02e
	sub (ix+002h)		;f030
	ld c,a			;f033
	ex af,af'			;f034
...
L_F03A:
	push de			;f03a
	ldi		;f03b   ; dieciseis `ldi` seguidos, sin bucle: media fila del logotipo de una tacada
	ldi		;f03d
	ldi		;f03f
	ldi		;f041
	ldi		;f043   ; la fila se copia entera de golpe, ocho `ldi` por cada mitad
	ldi		;f045
	ldi		;f047
	ldi		;f049
	ldi		;f04b
	...
	ldi		;f059
	pop de			;f05b
	add a,c			;f05c   ; al desplazamiento se le suma C, y de ahi sale si la fila ya esta llena
	cp 010h		;f05d
	jr nc,L_F069		;f05f
	ex af,af'			;f061
	...
	scf			;f068
L_F069:
	dec b			;f069   ; con B a cero se acabaron las dieciseis filas
	jr z,L_F077		;f06a
	and a			;f06c
	sbc a,010h		;f06d
	ex af,af'			;f06f
...
logo_borra:		; Pone a cero (ix+004) filas de 16 B desde DE, con `ld (hl),000h` y quince `ldi` desenrollados que arrastran el cero. Con (ix+004) a cero vuelve sin tocar nada
	ld a,(ix+004h)		;f07b   ; el borrado: la fila se corre un byte a la derecha metiendo un cero por delante
	and a			;f07e
	ret z			;f07f
L_F080:
...
logo_avanza:		; Cierra una fila del montaje: suma 16 a DE y sube el contador de filas de (ix+005), que es lo que luego mira logo_vuelca
	push hl			;f0ae   ; avanzar de fila son 0x10 bytes en el buffer, y ademas se cuenta en 0xF125
	ld hl,00010h		;f0af
	add hl,de			;f0b2
	ex de,hl			;f0b3
	pop hl			;f0b4
...
logo_vuelca:		; Suelta el montaje de 0x4000 a la VRAM transponiendolo: 16 columnas y, en cada una, las filas que diga (ix+005), leyendo a saltos de 16 en la RAM y bajando por la VRAM
	ld c,010h		;f0bd   ; el volcado: dieciseis columnas, cada una con sus filas
	ld de,04000h		;f0bf
L_F0C2:
	...
	push hl			;f0ca
L_F0CB:
	ld a,(de)			;f0cb   ; un byte por vuelta al puerto 0x98, saltando 0x10 bytes cada vez: se vuelca por columnas y no por filas
	out (098h),a		;f0cc
	ld a,e			;f0ce
	add a,010h		;f0cf
	ld e,a			;f0d1
	...
	adc a,000h		;f0d3
	ld d,a			;f0d5
	call vram_avanza_fila		;f0d6   ; y la fila siguiente del buffer
	djnz L_F0CB		;f0d9
	pop hl			;f0db
	ld de,00040h		;f0dc
	add hl,de			;f0df
...
L_F0EB:
	ex af,af'			;f0eb   ; el `ex af,af'` guarda el valor mientras se pone la direccion
	out (098h),a		;f0ec
	ex af,af'			;f0ee
	dec bc			;f0ef
	ld a,b			;f0f0
...
L_F0FA:
	ld a,(de)			;f0fa   ; y aqui se escribe
	out (098h),a		;f0fb
	inc de			;f0fd
	dec bc			;f0fe
	ld a,b			;f0ff
...
vram_pon_dir_lee:		; Fija la direccion de LECTURA del VDP por el puerto 0x99, sin el bit 0x40: es la gemela de vram_pon_dir
	di			;f114   ; poner la direccion para leer: sin el bit 6
	in a,(099h)		;f115
	ld a,l			;f117
	out (099h),a		;f118
	ld a,h			;f11a
...
apaga_barra_resto:		; Recupera de la pila los segmentos que faltan hasta 20 y los pinta de negro sobre negro, que es como se borran
	...
	ld b,a			;f33d
L_F33E:
	di			;f33e   ; la direccion de VRAM se pone a mano, sin pasar por la BIOS
	ld a,l			;f33f
	out (099h),a		;f340
	and a			;f342
	ld a,h			;f343
...
premia:		; Suma B al marcador por el digito que apunte HL (dentro de los seis de 0xDD80) y lo repinta en 0x12B0: es suma_puntos con IX y DE ya puestos, y salvando IX
	push ix		;f394   ; el premio de 0x12B0 puntos, sumado a la tabla de 0xDD80
	ld ix,0dd80h		;f396
	ld de,012b0h		;f39a
...
L_F6DD:
	dec bc			;f6dd   ; el bucle de espera del teclado: una cuenta vacia
	ld a,b			;f6de
	or c			;f6df
	jr nz,L_F6DD		;f6e0
	pop hl			;f6e2
...
pide_nombre_borra:		; El DEL: borra la celda, retrocede una columna y repinta alli el cursor
	pop de			;f6e6   ; el DEL: si no se ha escrito nada, no hay nada que borrar
	pop bc			;f6e7
	ld a,b			;f6e8
	push bc			;f6e9
	push de			;f6ea
...
carga_parte2:		; Carga la segunda parte desde la cinta y salta a ella. La cabecera de ocho bytes NO son datos: son las dos instrucciones que hay debajo, y se cargan ENCIMA de ellas para luego ejecutarse
	...
	ld a,081h		;f722
	out (099h),a		;f724
	call borra_buffer		;f726   ; se borra el buffer y se vuelca antes de encender
...
L_F7BA:
	...
	ld bc,00000h		;f7bb
L_F7BE:
	dec bc			;f7be   ; otra cuenta vacia, la del retardo del cargador
	ld a,b			;f7bf
	or c			;f7c0
	jr nz,L_F7BE		;f7c1
	pop bc			;f7c3
...
carga_cinta:		; La carga de la segunda parte: empuja a mano la vuelta a 0xF89F, arranca el motor (0xAB) y el registro 14 del PSG, y engancha el tono guia midiendo pulsos contra 0x9C y 0xC6. Es LD-BYTES de la ROM del Spectrum, con la lectura del bit adaptada al PSG del MSX
	ld hl,carga_vuelta		;f7f6   ; EL CARGADOR DE CINTA, escrito a mano y sin BIOS
	push hl			;f7f9
	push af			;f7fa
	ld a,008h		;f7fb
	out (0abh),a		;f7fd   ; el puerto 0xAB pone el motor del casete...
	ld a,00eh		;f7ff
	out (0a0h),a		;f801   ; ...y el 0xA0 con 0x0E deja el registro 14 del PSG a la escucha
	pop af			;f803
	inc d			;f804
	ex af,af'			;f805
	dec d			;f806
	di			;f807
	ld a,005h		;f808   ; se busca la cabecera midiendo pulsos: los umbrales son 0x9C, 0xC6, 0xC9 y 0xD4
	ld c,a			;f80a
	cp a			;f80b
L_F80C:
	...
	ld hl,00415h		;f811
L_F814:
	djnz L_F814		;f814   ; una cuenta larga de guarda antes de empezar a medir
	dec hl			;f816
	ld a,h			;f817
	or l			;f818
	jr nz,L_F814		;f819
	...
	jr nc,L_F80C		;f81e
L_F820:
	ld b,09ch		;f820   ; el tren de pulsos largos de la cabecera
	...
	jr nz,L_F820		;f82d
L_F82F:
	ld b,0c9h		;f82f   ; y el de los cortos, que es el que cierra
	call lee_bit_cinta		;f831
	jr nc,L_F80C		;f834
	ld a,b			;f836
	cp 0d4h		;f837   ; 0xD4 es el umbral del segundo tren
	jr nc,L_F82F		;f839
	...
	jr L_F85D		;f843
L_F845:
	ex af,af'			;f845   ; cada byte leido va al destino, y el bit de paridad se lleva en C
	jr nz,L_F84D		;f846
	ld (ix+000h),l		;f848
	jr L_F857		;f84b
L_F84D:
	rr c		;f84d   ; el bit leido se mete en C, que lleva la paridad
	xor l			;f84f
	ret nz			;f850
	ld a,c			;f851   ; y C se corre para el bit siguiente
	rla			;f852
	ld c,a			;f853
	inc de			;f854
	jr L_F859		;f855
...
L_F85D:
	ld l,001h		;f85d
L_F85F:
	call lee_bit_cinta2		;f85f   ; un bit por vuelta: se mide el pulso y se mete en L con `rl`
	ret nc			;f862
	ld a,0cbh		;f863
	cp b			;f865
	rl l		;f866   ; 0xCB es el umbral que separa el cero del uno
	ld b,0b0h		;f868
	jp nc,L_F85F		;f86a
	ld a,h			;f86d
	xor l			;f86e
	...
	or e			;f871
	jr nz,L_F845		;f872
	ld a,h			;f874   ; al final se comprueba la paridad: tiene que dar 1
	cp 001h		;f875
	ret			;f877
lee_bit_cinta2:		; Dos flancos seguidos con lee_bit_cinta, y vuelve en cuanto el primero falla
	call lee_bit_cinta		;f878   ; y aqui la espera entre bloque y bloque
	ret nc			;f87b
...
L_F87E:
	...
	and a			;f881
L_F882:
	inc b			;f882   ; el bucle de medida: se cuenta con B mientras el bit del puerto 0xA2 no cambie
	nop			;f883
	ret z			;f884
	ld a,000h		;f885
	in a,(0a2h)		;f887   ; el puerto 0xA2 es donde entra la senal del casete
	cpl			;f889
	xor c			;f88a
	and 080h		;f88b
	jp z,L_F882		;f88d
...
carga_vuelta:		; El final de la carga, y la direccion que carga_cinta se empuja a si misma en 0xF7F7: para el motor de la cinta y devuelve el borde a negro por el registro 7 del VDP, deshaciendo el `ld a,r` con que 0xF893 lo hace parpadear. No toca ni una bandera, para que el acarreo de exito llegue intacto
	ld e,013h		;f89f   ; y aqui se vuelve del cargador con las interrupciones otra vez abiertas
	ld a,009h		;f8a1
	out (0abh),a		;f8a3
	ld a,001h		;f8a5
	out (099h),a		;f8a7
